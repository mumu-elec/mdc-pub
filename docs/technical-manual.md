...
- **受保护配置区**：二进制协议中 config_t 前 12 字节（版本/校验信息）不可通过 WRITE_FIELD 写入
...
- [/smap - SBUS 通道映射](#cmd-smap)
- [/sbusparam - SBUS 行程映射](#cmd-sbusparam)
...
| 2 | 设置映射 | `/smap 1 3` → 电机 A 绑定 SBUS CH3 |
...
| 1 | 设置范围 | `/sbusrange 172 1811` → `OK (RAM only)` |
...
| `0x10` | READ_PARAM | 无 | config_t (253B) | 读取全部配置 |
| `0x11` | WRITE_PARAM | config_t (253B) | ACK | 写入全部配置（仅 RAM，受保护字段自动还原） |
...
### 5.7 配置数据结构（config_t，247B，v2.x）

offset 0~10 为受保护区域（魔数、硬件版本 A/B/C、固件协议版本 D、Bug 修复 E、保留位、CRC8），不可写入。可写字段偏移参考（v2.x 布局：删除 speed_fb_unit，速度环恒输入轴闭环）：
...
| sbus_param[4] | 211 | 8B | 遥控行程 (u16×4): 开环=最大PWM, 速度=最大RPM(输入轴口径), 位置=最大角度(0.1°) |
| sbus_range_min | 219 | 2B | 通道值下边界 (u16) |
| sbus_range_max | 221 | 2B | 通道值上边界 (u16) |
| chassis_type | 223 | 1B | 底盘构型 (0=禁用, 1~6 见指令参考) |
| wheel_diameter | 224 | 2B | 轮径 (mm) |
| chassis_geo[3] | 226 | 6B | 底盘几何 (mm/0.1°) |
| chassis_max[3] | 232 | 6B | 底盘限幅 (vx/vy mm/s, w 0.001rad/s) |
| chassis_accel[3] | 238 | 6B | 底盘加速度 (vx/vy mm/s², w 0.001rad/s²) |
| stall_time_ms | 244 | 2B | 失速判定时长 (ms, 0=禁用) |
| motor_map | 246 | 1B | 电机映射 (每电机2bit: 0=A 1=B 2=C 3=D) |

> **v2.x 布局变更**：删除 speed_fb_unit（速度命令单位, 1B），sizeof 248B → 247B，EEPROM 余 9B。
> **建议：** 上位机程序优先使用 READ_PARAM → WRITE_PARAM 全量读写，避免依赖 WRITE_FIELD 硬编码偏移（每次 SW_MAJOR 升级布局可能变化）。
> **注：** 头部由 v2.0 的 12B 变为 11B（删除 _pad）。Bootloader 的 write_hw_version 仍写前 12B（offset 11 填 0xFF 作 CRC 无效标记），会覆盖 baud_rate 低字节，但 BL 故意写坏 CRC → 固件 config_load() 走 defaults 重置兜底，实际无害。
...
| BL | Bootloader |
| config_t | 247 字节配置结构体（EEPROM 持久化，v2.x） |
