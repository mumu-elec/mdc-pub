...
> **手册版本：** v1.0  
> **适用范围：** 硬件 v**1.1**.x | 固件 v**1.0**.x | 上位机 v**1.0**.x  
...
offset 0~10 为受保护区域（魔数、硬件版本 A/B/C、固件协议版本 D、Bug 修复 E、保留位、CRC8），不可写入。可写字段偏移参考（v1.0 位宽打包布局）：
...
| speed_filter_window[4] | 210 | 4B | 滤波窗口 (u8×4) |
| sbus_channel_pack | 214 | 2B | 每电机4bit: 0~15=CH1~16（必绑，无未绑定） |
| rc_dir_ch | 216 | 2B | 方向映射通道（每电机4bit: 0~15=CH1~16） |
| rc_map_mode | 218 | 1B | bit0-3=映射模式(0=中心零点,1=min零点), bit4-7=方向映射使能(每电机1bit) |
| sbus_param[4] | 219 | 8B | 遥控行程 (u16×4): 开环=最大PWM, 速度=最大RPM, 位置=最大角度(0.1°) |
| sbus_range_min | 227 | 2B | 通道值下边界 (u16) |
| sbus_range_max | 229 | 2B | 通道值上边界 (u16) |
...
| BL | Bootloader |
| config_t | 250 字节配置结构体（EEPROM 持久化） |
