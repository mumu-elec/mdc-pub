# Motor Driver Controller — 技术手册

> **产品：** 基于 STM32F401 + TB6612 的四路带编码器直流电机驱动器  
> **手册版本：** v2.0  
> **适用范围：** 硬件 v**1.1**.x | 固件 v**1.2**.x | 上位机 v**2.0**.x  
> **版本说明：** [5.8 版本体系](#58-版本体系)

---

> **修订版本：** v2.0  
> **修订时间：** 2026-09-11  
> **修订记录：** [附录D. 手册修订记录](#d-手册修订记录)

---

<div align="center"><img src="image/3D_PCB2_2026-08-04.png" width=400></div>

---

## 目录

- [1. 产品概述](#1-产品概述)
- [2. 技术规格与电气属性](#2-技术规格与电气属性)
- [3. 硬件接口与引脚定义](#3-硬件接口与引脚定义)
- [4. 固件功能说明](#4-固件功能说明)
- [5. 通信协议](#5-通信协议)
- [6. 快速上手](#6-快速上手)
- [7. 固件更新（OTA）](#7-固件更新ota)
- [8. 故障排查](#8-故障排查)
- [9. 附录](#9-附录)

---

## 1. 产品概述

Motor Driver Controller 是四路直流电机驱动器，内置 PID 闭环算法，支持 SBUS / ELRS（CRSF）遥控器与 UART 串口三种控制方式，可通过浏览器（Web Serial）进行参数配置、实时监控与 OTA 固件升级。

| 特性 | 说明 |
|------|------|
| 电机通道 | 4 路独立直流电机（2× TB6612FNG 双路 H 桥） |
| 编码器接口 | 4 路正交编码器，硬件 4× 倍频 |
| 控制协议 | SBUS（100kbps 8E2）/ ELRS·CRSF（420kbps 8N1）/ UART 文本指令 / UART 二进制帧 |
| 闭环模式 | 开环 / 速度闭环 / 位置闭环，每通道独立 |
| PID 类型 | 位置式 / 增量式，每通道可选（速度环、位置环独立配置） |
| 配置存储 | 板上 EEPROM 保存全部配置，开机自动加载 |
| 调试接口 | USB Type-C 虚拟串口（CH340N）+ SWD 4-Pin |
| 上位机 | Web 上位机（开源），浏览器直连，支持参数配置 / 实时曲线 / OTA 刷写 |

---

## 2. 技术规格与电气属性

### 2.1 电气规格

| 参数 | 数值 |
|------|------|
| 主控芯片 | STM32F401RCT6（Cortex-M4, 84MHz, 64KB SRAM, 256KB Flash） |
| 电机驱动 | 2× TB6612FNG，单路持续 1.2A，峰值 3.2A |
| 输入电压 | DC 5.5V ~ 15V（推荐 7.4V / 11.1V / 12V） |
| 电源极性 | DC-005 2.1mm 插座 |
| 电源架构 | DC IN → LGS5145 Buck → 5V → 662K LDO → 3.3V |
| 逻辑供电 | 3.3V（内部 LDO） |
| 编码器供电 | 3.3V（由板载 LDO 提供） |
| 控制信号电平 | 3.3V / 5V TTL |
| 存储 | AD24C02，2K-bit（256B）I2C EEPROM |
| PWM 频率 | 20kHz（TIM1） |
| 尺寸 | 60mm × 60mm |

### 2.2 电源要求

- **输入电压范围：** DC 5.5V ~ 15V（推荐 ≤12V）
- **电源接口：** DC-005 2.1mm（内正外负）
- **推荐电压：** 7.4V（2S 锂电）/ 11.1V（3S 锂电）/ 12V
- **无反向保护：** 板上无反接保护二极管，**接反、过压会损坏电路**，通电前务必确认极性

#### 极限参数（Ta=25°C）

| 对外接口 | 最小 | 最大 | 说明 |
|---------|:---:|:---:|------|
| 总电源输入（VIN） | 5.5V | 15V | 内正外负，无反接保护 |
| 电机输出 | 0V | VIN | 单路持续 1.2A / 峰值 3.2A（TB6612） |
| 编码器供电 | 3.0V | 3.6V | 3.3V 标称，≤300mA（板载 LDO 总量，与板上负载共享） |
| 编码器输入 | 0V | 3.6V | 3.3V 逻辑电平 |
| RC 控制信号输入 | 0V | 5.5V | 3.3V / 5V TTL，由板载异或门中继信号 |
| USB-C 输入 | / | / | Type-C 不参与板载供电 |
| 5V 输出 | 4.75V | 5.25V | 5V，**两路合计 ≤1A**（Buck 上限，与板上负载共享），仅用作传感器、MCU供电，请勿用作舵机等供电高瞬时功率电气供电 |

> **极限电压推导：** 最低输入 5.5V 由 LGS5145 Buck 输出 5V 的降压比下限决定（供电链最苛刻环节）；最高 15V 由 TB6612 VM 极限参数决定。供电链：DC IN → LGS5145 Buck(5V) → 662K LDO(3.3V)。

> **编码器**输入无中继保护，如编码器使用外接供电，请确保供电电源不高于5V

> **RC**接口由异或门中继，故可以接收3V3/5V电平信号

> **任意情况下，都请勿超过以上参数**


### 2.3 机械与连接器规格

| 连接器 | 规格 | 用途 |
|--------|------|------|
| A/B/C/D | 6-Pin XH 2.5mm 弯插母座 | 每通道电机 + 编码器 |
| RC | 2-Pin 1.5mm 卧贴母座 | SBUS / UART 控制信号（无极性输入） |
| USB | Type-C 母座 | 上位机连接 / 调试 / OTA（不参与板载供电） |
| DC IN | DC-005 2.1mm | 电源输入 |
| 5V 输出 ×2 | 独立端子 | 5V ≤1A 供电输出（传感器 / MCU，勿用于舵机等） |
| SWD | 4-Pin 1.27mm 排针 | 调试接口（3V3/SWDIO/SWCLK/GND） |

---

## 3. 硬件接口与引脚定义

### 3.1 板面布局

| 编号 | 名称 | 类型 | 说明 |
|------|------|------|------|
| ① | A / B / C / D | 6-Pin XH 2.5mm 母座 | 电机 + 编码器连接器 |
| ② | RC | 2-Pin 1.5mm 卧贴母座 | SBUS / UART 控制信号输入（无极性） |
| ③ | USB | Type-C 母座 | USB 调试 / 上位机连接 |
| ④ | DC IN | DC-005 2.1mm | 电源输入（内正外负） |
| ⑤ | SWD | 4-Pin 1.27mm 排针 | SWD 调试接口 |
| ⑥ | LED | 0603 红色 LED | 工作状态指示灯 |
| ⑦ | FUN | 轻触按键 | 见下方按键功能说明 |
| ⑧ | SW1 | 滑动开关 | 电源开关 |
| ⑨ | RST | 轻触按键 | 系统复位 |

**FUN 键功能：**
- 按住 FUN 上电 → 进入 Bootloader（OTA 模式）
- 运行中长按 1 秒 → 协议自动识别（SBUS/UART）

### 3.2 电机连接器（6-Pin XH，A/B/C/D）

| Pin | 名称 | 方向 | 说明 |
|-----|------|------|------|
| 1 | MOTOR_OUT2 | 输出 | TB6612 电机驱动线 2 |
| 2 | GND | — | 编码器电源地 |
| 3 | ENC_A | 输入 | 编码器 A 相信号 |
| 4 | ENC_B | 输入 | 编码器 B 相信号 |
| 5 | +3.3V | 输出 | 编码器供电 |
| 6 | MOTOR_OUT1 | 输出 | TB6612 电机驱动线 1 |

> **线序建议：** Pin3/Pin4 接反会导致编码器方向识别错误，可用 `/einv` 软件反转补偿。

### 3.3 控制信号输入（RC）

**无极性 2-Pin 1.5mm 卧贴插座**，GND 与 SIGNAL 可正反插，支持：
- **SBUS 接收机**：直接插入，FUN 键识别
- **ELRS 接收机**：CRSF 输出（420kbps 8N1，默认直通不反相），直接插入，FUN 键识别
- **UART TX 信号**：3.3V / 5V TTL 电平，接外部 MCU 的 TX 引脚（需共地）

> RC 接口由板上异或门中继，故可接收 3.3V / 5V 电平信号；RC 仅传信号，**不对外供电**；SBUS / ELRS 接收机需要外部供电（通常接接收机自己的电源）。

### 3.4 调试接口

- **USB Type-C**：CH340N 虚拟串口，固定 **2000000-8N1**，承载上位机通信（文本指令 + 二进制帧）与 OTA 刷写；**仅通信，不参与板载供电**（板载供电仅来自 DC IN）
- **SWD 4-Pin**：3V3 / SWDIO / SWCLK / GND，用于固件级调试

### 3.5 MCU 引脚分配

**电机驱动（IN1/IN2 方向 + PWM 20kHz）：**

| 通道 | IN1 | IN2 | PWM (TIM1) |
|------|-----|-----|------------|
| Motor A | PC12 | PC11 | PA9 (TIM1_CH2) |
| Motor B | PA12 | PC9 | PA8 (TIM1_CH1) |
| Motor C | PA15 | PC10 | PA10 (TIM1_CH3) |
| Motor D | PB4 | PD2 | PA11 (TIM1_CH4) |

**编码器（Encoder Mode，硬件 4× 倍频）：**

| 编码器 | 定时器 | CH1 | CH2 |
|--------|--------|-----|-----|
| Encoder A | TIM3 (16-bit) | PA6 | PA7 |
| Encoder B | TIM5 (32-bit) | PA0 | PA1 |
| Encoder C | TIM2 (32-bit) | PA5 | PB3 |
| Encoder D | TIM4 (16-bit) | PB6 | PB7 |

**通讯与其它：**

| 功能 | 引脚 | 说明 |
|------|------|------|
| 控制输入 | PA3 (USART2 RX) | SBUS / UART，经异或门无极性输入 |
| 极性控制 | PC13 (GPIO) | 异或门极性翻转（内部自动管理） |
| USB 调试 | PC6/PC7 (USART6) | 2000000 8N1 → CH340N → Type-C |
| EEPROM I2C | PB8 (SCL) / PB9 (SDA) | AD24C02 |
| LED | PA4 | 状态指示灯 |
| FUN 键 | PC0 (EXTI0) | 上电按住进 BL / 运行中长按重识别 |

---

## 4. 固件功能说明

### 4.1 控制模式（每通道独立）

| 模式 | 目标值含义 | 需要编码器 | 适用场景 |
|------|-----------|:--:|------|
| 开环（open） | 目标值直接作为 PWM 输出（±1000） | ❌ | 简单调速、测试 |
| 速度闭环（speed） | 目标值为 RPM | ✅ | 需要精确转速 |
| 位置闭环（pos） | 目标值为编码器位置（0.1° 精度） | ✅ | 相对位置控制 |

- 位置闭环为 **位置环 → 速度环级联** 结构
- 使用位置闭环时，应**先整定速度环**参数
- 编码器线数（CPR）设为 0 时该通道强制开环
- **底盘模式下（chassis_type≠0）**：每轮执行档位由该通道控制模式决定——开环=按标定系数 k_ch（`/sbusparam` 值 ×0.01）折算 PWM，速度闭环=速度环 PID；位置闭环档不参与底盘链路、按速度闭环执行（固件 ≥ v1.2.10；旧版固件在此场景忽略控制模式、有编码器的轮恒为速度闭环）。`/chassis` 查询的 `MODE` 行显示每轮实际执行档位。

### 4.2 编码器与速度滤波

- **CPR（每转物理刻度数）**：按编码器规格书设置（常见 11/13/500 线）；不确定时可用 `/check` 手动转一圈看计数增量，或者查看上位机状态显示。
- **位置环输出轴转一圈脉冲数**：`/posangle`，物理线数（固件自动 ×4），0 = 使用编码器 CPR 数值
- **速度滤波器**（每通道独立）：滤波作用于编码器 RPM 计算之后、速度环 PID 输入之前，目的是消除转速测量噪声。可选 4 种类型：

| type | 名称 | window 范围 |
|:--:|------|:--:|
| 0 | 无滤波 | — |
| 1 | 滑动平均 (MA) | 1~32 |
| 2 | 一阶低通 (LPF) | 1~99 |
| 3 | 中值滤波 (Median) | 1~32 |

> 通用原则：滤波窗口越大越平滑，但滞后/响应越差；闭环（速度/位置）下过大的滤波窗口会造成反馈滞后，导致振荡。建议从较小窗口（4~8）开始调整。下面分别详细介绍三种滤波器的原理、参数含义与影响。

#### 4.2.1 滑动平均（MA，type=1）

**原理：** 将最近 N 次原始 RPM 求算术平均作为输出，属于 FIR（有限脉冲响应）线性滤波。

**参数含义：** `window`（N，1~32）即参与平均的样本数——滤波窗口长度。

**公式：**

$$y[n] = \frac{x[n] + x[n-1] + \cdots + x[n-N+1]}{N}$$

**参数大小对结果的影响：**
- N=1：等于无滤波（y=x）
- N 增大：输出更平滑，对随机噪声的抑制更强；但同时引入约 (N-1)/2 个采样周期的固定滞后，阶跃响应变慢
- 滑动平均对**均匀随机噪声**效果最好，但对周期性干扰抑制有限

**应用场景：** 电机稳态运行时降低 RPM 抖动、实时性要求不高的场合（如速度显示、简单调速）。波形上输出平滑、无毛刺，但跟随目标速度变化有明显延迟。

#### 4.2.2 一阶低通（LPF，type=2）

**原理：** 一阶 IIR 低通滤波，输出依赖历史输出值，属于指数平滑。截止频率随窗口增大而降低。

**参数含义：** `window`（N，1~99）为滤波系数，**不是样本数量**——它决定平滑强度：越大则截止频率越低、滤波越强。

**公式：**

$$y[n] = y[n-1] + \frac{x[n] - y[n-1]}{N+1}$$

**参数大小对结果的影响：**
- N=1：等同无滤波（固件 w≤1 时透传）
- N≥2：输出越平滑、噪声抑制越强，但响应越慢，无固定窗口滞后（理论滞后随输入频率变化）
- N 过大时对突变目标响应迟钝，闭环中表现为"追不上"、积分饱和

**应用场景：** 兼顾响应与平滑的通用选择，闭环（速度环）前常用。波形上平滑且无窗口阶梯感，适合需要一定平滑又要求实时性的场景。

#### 4.2.3 中值滤波（Median，type=3）

**原理：** 将最近 N 次原始 RPM 排序后取中间值作为输出，属于非线性滤波。

**参数含义：** `window`（N，1~32）即参与排序的样本数（窗口长度）。

**公式：**

$$y[n] = \mathrm{median}(x[n],\, x[n-1],\, \ldots,\, x[n-N+1])$$

**参数大小对结果的影响：**
- N 增大：对孤立脉冲/尖峰噪声的免疫越强（单个异常值不会影响中值）
- 但对连续跳变的平滑能力不如 MA/LPF，且输出呈"阶梯状"；N 为偶数时取中间两值之一
- 计算开销随 N 增大（需排序）

**应用场景：** 信号存在偶发毛刺、跳变时首选（如编码器单点毛刺、接触不良产生的尖峰）。波形上尖峰被完全剔除，但真实转速快速变化时会有阶梯感。

#### 三种滤波器对比

| 特性 | 滑动平均 (MA) | 一阶低通 (LPF) | 中值滤波 (Median) |
|------|------|------|------|
| 线性 | 是（FIR） | 是（IIR） | 否 |
| 抑制对象 | 均匀随机噪声 | 高频噪声 | 脉冲/尖峰噪声 |
| 滞后 | 固定约 (N-1)/2 周期 | 随输入变化，无固定窗口 | 约 (N-1)/2 周期 |
| 响应速度 | 中 | 快（窗口小时） | 快（无噪声时） |
| 平滑程度 | 好 | 最好（窗口大时） | 一般（阶梯状） |
| 抗尖峰 | 差 | 中 | **最好** |
| 推荐场景 | 稳态测速 | 闭环通用 | 毛刺干扰明显时 |

### 4.3 PID 控制

**速度环参数**（`/speedctrl`）：Kp / Ki / Kd、积分限幅（Ilim）、输出限幅（Olim，0~1000，钳位 PWM）、计算周期（默认 10ms）、PID 类型（0=位置式，1=增量式）。

**位置环参数**（`/posctrl`）：Kp / Ki / Kd、输出限幅（RPM）、积分限幅（0.1° 单位）、计算周期（默认 20ms）、PID 类型。

**速度闭环调参推荐顺序：**
1. 从较小 Kp 开始（如 `Kp=0.3, Ki=0, Kd=0`）
2. 给目标转速（如 100 RPM），观察实际 RPM 曲线
3. 逐步增大 Kp 直到响应快速但有轻微过冲
4. 实际转速偏低（稳态误差）→ 逐步增大 Ki
5. 出现震荡 → 增大 Kd 或减小 Kp
6. 满意后 `/save` 写入 EEPROM

**参考值：** `/speedctrl 1 0.5 0.02 0.01 500 800 10 0`

### 4.4 安全机制

- **命令超时保护**（`/timeout`）：超过设定时间未收到控制指令，所有电机输出自动归零；0 = 关闭保护
- **控制优先级**（`/priority`）：
  - `0` = USART2 优先（遥控器主控，默认）
  - `1` = USB 优先（上位机主控）
  - 优先端口失联超过心跳窗口（`/timeout` 值，最小 100ms）后，非优先端口自动接管；优先端口恢复后立即夺回。`/timeout 0` 仅关闭输出归零，仲裁仍按最小 100ms 窗口让位
- **受保护配置区**：二进制协议中 config_t 前 11 字节（offset 0~10，魔数/版本/CRC 校验信息）不可通过 WRITE_FIELD 写入（详见 §5.7）

### 4.5 遥控控制（SBUS / ELRS）

- **协议说明**：SBUS 与 ELRS（CRSF 格式）是两种独立遥控串行协议（关系详见 `docs/rc-protocols.md`），通道映射/映射模式/方向映射/行程映射/通道值域范围设置对两者完全共用。
- **默认通道映射**：CH1~CH4 → 电机 A~D（可用 `/smap` 修改，同一遥控通道可映射到多个电机；每电机必须绑定通道，无未绑定状态，不用该路就物理不接电机）
- **映射模式**（`/rmap`，v1.0 支持）：每电机可独立选择
  - 中心零点（默认，双向）：以通道中心为 0 点，`norm=(ch−中心)/半幅 ∈ [−1,+1]`，摇杆居中=0
  - min零点（单向）：以下边界为 0 点，`norm=(ch−min)/(max−min) ∈ [0,+1]`，只输出正向行程，配合方向映射实现正反转
- **方向映射**（`/dmap`，v1.0 支持）：每电机绑定一个开关通道控制正反转（只映射遥控信号方向，不改物理引脚方向，物理方向仍由 `/inv` 控制）
  - 方向通道在中心（±16 容差）→ 关闭电机（输出 0）
  - 方向通道在上边界 → 正转；下边界 → 反转
  - 两档开关 = 恒正/恒反；三档开关 = 正转/停止/反转
  - **底盘模式（v1.2.4 起生效）**：方向映射按运动向量轴解释——`/dmap 1/2/3` 对应 vx/vy/ω（4 预留无效），方向通道在中心 → 该轴关闭（修复：此前底盘分支漏掉方向映射，仅单电机路径生效）
- **默认反转**（`/dinv`，v1.2.12 新增）：通道级**静态**方向反转，配置后上电即生效，不依赖遥控器开关（适配不同遥控器的摇杆方向约定）；电机模式 ch1~4 = 电机 A~D，底盘模式 ch1~3 = vx/vy/ω。与方向映射**乘法叠加**：`电机速度 = 底层计算速度 × 默认反转(±1) × 方向开关(±1/0)`；只映射遥控信号方向，不改 `/inv` 物理引脚方向，上位机 0x31/0x30 直控不受影响
- **行程映射**（`/sbusparam`）：摇杆满偏对应目标值，含义取决于控制模式：
  - 开环：最大 PWM（0~1000，默认 1000，可配置）
  - 速度闭环：最大 RPM（默认 10000）
  - 位置闭环：最大角度（0.1° 单位，3600 = ±360.0°）
- **通道值范围**（`/sbusrange`）：固件只存上下界 min/max，归一化中心=(min+max)/2、半幅=(max-min)/2。上位机下拉提供快捷预设（标准 192~1792 / 扩展 0~2047 / CRSF·ELRS 172~1811），选中即写入对应 min/max，也可自定义输入。合法域 100 ≤ min < max ≤ 2047。

### 4.6 协议自动识别

上电时使用 EEPROM 中保存的协议配置，**不会**自动运行识别。触发方式：
- FUN 键长按 >1 秒
- `/detect` 指令（上位机「自动识别 USART2 串口」按钮即发送此指令）

识别流程：阶段 1 尝试 SBUS（100kbps 8E2，验证帧头 0x0F 与帧尾 0x00）；阶段 2 尝试 ELRS/CRSF（420kbps 8N1，验证 0xC8 帧头 + CRC8-DVB-S2）；阶段 3 尝试 UART（自动扫描 12 档常用波特率 × 2 极性）。识别成功后保存到 EEPROM 并重启协议。识别结果通过 `DETECT_REPORT`（0xF1）帧上报协议类型（proto=1/2/3 分别对应 SBUS/UART/ELRS）、信号极性与波特率（见 §5.5）。

### 4.7 配置持久化

- 所有参数修改**仅写入 RAM 即时生效**，需 `/save`（或上位机「保存到 EEPROM」）才持久化
- 开机自动从 EEPROM 加载配置
- `/reset` 恢复出厂默认值
- `/load` 从 EEPROM 重新加载（丢弃 RAM 中未保存的修改）

### 4.8 底盘功能（v2.0 新增）

底盘模式把四路电机组织为一个整车运动模型：上位机/遥控/串口指令给出整车速度向量 **(vx 前进， vy 平移， ω 旋转)**，固件完成限幅 → 梯形加减速 → 失速协调 → 运动学逆解，输出每轮目标转速。

#### 构型一览（chassis_type）

| 值 | 构型 | 轮位约定（模型轮位 → 电机） |
|:--:|------|------|
| 0 | 禁用（单电机模式） | — |
| 1 | 差速 2WD | A=左， B=右 |
| 2 | 差速 4WD 四驱 | A=左前 B=右前 C=左后 D=右后（同侧同速） |
| 3 | 麦轮 O 型 | A=左前 B=右前 C=左后 D=右后（辊子 −45°/+45°/+45°/−45°） |
| 4 | 全向 3WD | A/B/C = 120° 品字布局 1/2/3 号轮 |
| 5 | 全向 4WD X 型 | 四轮四角，轮位角 45°（θ0=45°） |
| 6 | 全向 4WD 十字型 | 四轮四角，轮位角 0°（θ0=0°，左前/右后横滚 vy，右前/左后纵滚 vx） |

> 轮位与实际接线不一致时，优先用 `/inv` `/einv` 修正单轮方向；也可用**电机映射**（motor_map，上位机「底盘设置」区）把模型轮位重映射到任意物理接线口（默认 A→A B→B C→C D→D 顺序）。

#### 底盘参数（`/chassis` 配置，详见 §5.2 指令参考）

| 参数 | 单位 | 说明 |
|------|------|------|
| 轮径 wheel_diameter | mm | 速度换算基础，务必与实际轮径一致 |
| 几何 geo[3] | mm / 0.1° | 随构型含义不同：差速=轮距/轴距；麦轮=轴距/轮距；全向=半径/基准角 θ0 |
| 限幅 max[3] | vx/vy: mm/s，ω: 0.001 rad/s | **一份值三用**：遥控摇杆满偏目标、串口指令限幅、斜坡上限 |
| 加速度 accel[3] | vx/vy: mm/s²，ω: 0.001 rad/s² | 梯形斜坡限斜率，分轴独立；急停按斜坡减速、不硬切断 |
| 失速判定 stall_time_ms | ms | 0=禁用；失速协调按最慢轮能力比缩放整车目标（下限 10% 蠕动，不切断不锁死），持续超时置失速告警 |

#### 执行档位与减速比

- 底盘模式下每轮执行档位由该通道**控制模式**决定（§4.1）：开环 = 轮目标 RPM × 标定系数 k_ch（`/sbusparam` 值 × 0.01）折算 PWM，速度闭环 = 速度环 PID，位置档按速度闭环执行。选择构型不会改写各通道控制模式。
- **减速比 g = 输出轴一圈线数 ÷ 编码器线数**（`/posangle` 配置）。**g 未配置（=1）时实车速度会慢 g 倍**（如 30:1 减速箱未配置，车速慢 30 倍）。`/chassis` 查询的 `GEAR` 行显示每轮 g 并提示未配置的轮数。
- `/chassis` 查询的 `MODE` 行显示每轮实际执行档位；开环轮 k_ch 仍为出厂默认 1000（未标定）时带 `(k_ch未标定?)` 标注——非零目标会饱和到满 PWM，上车前请先标定或改速度闭环。

#### 控制方式

| 途径 | 方法 |
|------|------|
| 上位机 | 「底盘设置」区选构型预设 → 配置轮径/几何/限幅/加速度 → 控制面板推杆发 (vx, vy, ω)；电机映射示意图支持**长按轮子低速点动验向**（0x32 单轮点动） |
| 遥控器 | 遥控通道映射的 ch1/ch2/ch3 分别对应 vx/vy/ω（满偏 = 限幅值）；`/dmap` `/dinv` 在底盘模式按运动向量轴解释（见 §4.5） |
| 串口编程 | 二进制 `MOTOR_CTRL (0x31)` 帧发整车向量：p1=vx (mm/s)、p2=vy (mm/s)、p3=ω (0.001 rad/s)、p4 保留（见 §5.5；例程见 §6.6） |

> **注意：** 位置闭环不参与底盘链路；底盘模式下 `MOTOR_RAW (0x30)` 的单通道 PWM 会被底盘逆解每 1ms 覆盖，验向请用 0x32 点动或控制面板长按轮子。

---

## 5. 通信协议

### 5.1 串口参数

| 端口 | 指令集 |
|------|------|
| USART6 (USB)  | 文本指令集、字节指令集 |
| USART2 (控制端)| 文本指令集、字节指令集、SBUS协议 |

### 5.2 文本指令集（27 条）

**指令目录：**

- [/help - 打印所有可用命令](#cmd-help)
- [/version - 显示硬件/软件版本](#cmd-version)
- [/status - 打印全部配置参数](#cmd-status)
- [/check - 显示实时状态（编码器/RPM/输出/运行时间）](#cmd-check)
- [/detect - 重新检测 USART2 工作模式/波特率](#cmd-detect)
- [/save - RAM 配置写入 EEPROM](#cmd-save)
- [/load - 从 EEPROM 重新加载配置到 RAM](#cmd-load)
- [/reset - 恢复出厂默认值](#cmd-reset)
- [/speedctrl - 速度控制器](#cmd-speedctrl)
- [/posctrl - 位置控制器](#cmd-posctrl)
- [/mode - 控制模式](#cmd-mode)
- [/cpr - 编码器线数](#cmd-cpr)
- [/inv - 电机方向反转](#cmd-inv)
- [/einv - 编码器极性反转](#cmd-einv)
- [/posangle - 位置环转一圈脉冲数](#cmd-posangle)
- [/enczero - 编码器清零](#cmd-enczero)
- [/filter - 速度滤波器](#cmd-filter)
- [/uart2 - USART2 配置](#cmd-uart2)
- [/priority - 控制优先级](#cmd-priority)
- [/timeout - 指令超时保护](#cmd-timeout)
- [/smap - 遥控通道映射](#cmd-smap)
- [/rmap - 映射模式](#cmd-rmap)
- [/dmap - 方向映射](#cmd-dmap)
- [/dinv - 遥控默认反转](#cmd-dinv)
- [/sbusparam - 遥控行程映射](#cmd-sbusparam)
- [/sbusrange - 通道值范围上下界](#cmd-sbusrange)
- [/chassis - 底盘构型与参数](#cmd-chassis)

**通用规则：**
- 带参数 = 写入（仅修改 RAM，即时生效）；不带参数 = 读取
- 通道编号 `ch`：1~4 对应 Motor A~D
- 写入后需 `/save` 才能持久化到 EEPROM（`/uart2` 除外：设置后立即生效并应用）
- 成功写入返回 `OK (RAM only)`
- **可选参数省略规则：** 参数按 `[kp ki kd [ilim [olim ...]]]` 顺序依次可选，省略的尾部参数保持当前值不修改（如 `/speedctrl 1 0.5 0.02 0.01` 只改 Kp/Ki/Kd）；要设置靠后的参数必须先给出前面的参数；部分参数在特定配置下被忽略（见各指令参数表说明）

<a id="cmd-help"></a><a id="cmd-version"></a><a id="cmd-status"></a><a id="cmd-check"></a><a id="cmd-detect"></a><a id="cmd-save"></a><a id="cmd-load"></a><a id="cmd-reset"></a>
#### 系统指令（8 条）

| 指令 | 参数 | 说明 |
|------|------|------|
| `/help` | 无 | 打印所有可用命令 |
| `/version` | 无 | 显示硬件/软件版本 |
| `/status` | 无 | 打印全部配置参数 |
| `/check` | 无 | 显示实时状态（编码器/RPM/输出/运行时间） |
| `/detect` | 无 | 重新检测 USART2 工作模式/波特率 |
| `/save` | 无 | RAM 配置写入 EEPROM |
| `/load` | 无 | 从 EEPROM 重新加载配置到 RAM |
| `/reset` | 无 | 恢复出厂默认值 |

#### 控制器指令（6 条）

<a id="cmd-speedctrl"></a>
##### **/speedctrl** - 速度控制器

**`/speedctrl <ch> [kp ki kd [ilim [olim [period [ptype]]]]]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| kp | 比例增益 | 必填（与 ki/kd 同设） |
| ki | 积分增益 | 必填（与 kp/kd 同设） |
| kd | 微分增益 | 必填（与 kp/ki 同设） |
| ilim | 积分限幅 | 可省略；**增量式 PID（ptype=1）下忽略** |
| olim | 输出限幅（0~1000，钳位 PWM） | 可省略 |
| period | 计算周期（ms） | 可省略 |
| ptype | PID 类型：0=位置式，1=增量式 | 可省略 |

> **参数省略说明：** `ilim`/`olim`/`period`/`ptype` 均为可选，省略则保持当前值。但 `kp/ki/kd` 三者必须同时给出（不能只设其中一个）。
>
> **增量式 PID 下 `ilim` 的处理：** 增量式（ptype=1）无积分累加，`ilim` 不参与计算——可随意填一个值占位（如 0 或默认 500），不影响运行；但**不能跳过该参数位置**：若要设置其后的 `olim`/`period`/`ptype`，必须把 `ilim` 一并给出。建议填入合理值而非 0，因为切换回位置式（ptype=0）后 `ilim` 立即生效，填 0 会导致积分无法累积。

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 2 | 读取通道参数 | `/speedctrl 1` → `CH1 Kp=0.500 Ki=0.020 Kd=0.010 Ilim=500.0 Olim=1000.0 Period=10ms Type=位置式` |
| 5 | 设 Kp/Ki/Kd | `/speedctrl 1 0.5 0.02 0.01` |
| 6 | + 积分限幅 | `/speedctrl 1 0.5 0.02 0.01 500` |
| 7 | + 输出限幅 | `/speedctrl 1 0.5 0.02 0.01 500 800` |
| 8 | + 计算周期 | `/speedctrl 1 0.5 0.02 0.01 500 800 10` |
| 9 | + PID 类型 | `/speedctrl 1 0.5 0.02 0.01 500 800 10 0`（0=位置式，1=增量式） |

<a id="cmd-posctrl"></a>
##### **/posctrl** - 位置控制器

**`/posctrl <ch> [kp ki kd [olim [ilim [period [ptype]]]]]`**

位置闭环外环，0.1° 精度（3600 = ±360.0°）。

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| kp | 比例增益 | 必填（与 ki/kd 同设） |
| ki | 积分增益 | 必填（与 kp/kd 同设） |
| kd | 微分增益 | 必填（与 kp/ki 同设） |
| olim | 输出限幅（RPM） | 可省略 |
| ilim | 积分限幅（0.1° 单位） | 可省略；**增量式 PID（ptype=1）下忽略** |
| period | 计算周期（ms） | 可省略 |
| ptype | PID 类型：0=位置式，1=增量式 | 可省略 |

> **参数省略说明：** `olim`/`ilim`/`period`/`ptype` 均为可选，省略则保持当前值。但 `kp/ki/kd` 三者必须同时给出。
>
> **增量式 PID 下 `ilim` 的处理：** 增量式（ptype=1）无积分累加，`ilim` 不参与计算——可随意填一个值占位（如 0 或默认 500），不影响运行；但**不能跳过该参数位置**：若要设置其后的 `period`/`ptype`，必须把 `ilim` 一并给出。建议填入合理值而非 0，因为切换回位置式（ptype=0）后 `ilim` 立即生效，填 0 会导致积分无法累积。

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 2 | 读取通道参数 | `/posctrl 1` → `CH1 pos Kp=1.000 Ki=0.010 Kd=0.005 Olim=300.0RPM Ilim=500.0 Period=20ms PID=位置式` |
| 5 | 设 Kp/Ki/Kd | `/posctrl 1 1.0 0.01 0.005` |
| 6 | + 输出限幅 | `/posctrl 1 1.0 0.01 0.005 300` |
| 7 | + 积分限幅 | `/posctrl 1 1.0 0.01 0.005 300 500` |
| 8 | + 计算周期 | `/posctrl 1 1.0 0.01 0.005 300 500 20` |
| 9 | + PID 类型 | `/posctrl 1 1.0 0.01 0.005 300 500 20 0` |

<a id="cmd-mode"></a>
##### **/mode** - 控制模式

**`/mode <ch> [open|speed|pos]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| open | 开环：目标值直接作为 PWM 输出 | 可省略（不传=读取） |
| speed | 速度闭环：目标值为 RPM，由速度控制器计算 PWM | 可省略（不传=读取） |
| pos | 位置闭环：目标值为编码器位置，由位置环→速度环级联计算 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取通道模式 | `/mode 1` → `CH1 mode=OPEN` |
| 2 | 设置模式 | `/mode 1 speed` → `OK (RAM only)` |

<a id="cmd-cpr"></a>
##### **/cpr** - 编码器线数

**`/cpr <ch> [val]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| val | 编码器每转线数（CPR）；设为 0 强制开环 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取通道 CPR | `/cpr 1` → `CH1 CPR=1560` |
| 2 | 设置 CPR | `/cpr 1 600` → `OK (RAM only)` |

<a id="cmd-inv"></a>
##### **/inv** - 电机方向反转

**`/inv <ch> [0|1]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| val | 0=正常，1=反转 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取反转状态 | `/inv 1` → `CH1 invert=0` |
| 2 | 设置反转 | `/inv 1 1` → `OK (RAM only)` |

<a id="cmd-einv"></a>
##### **/einv** - 编码器极性反转

**`/einv <ch> [0|1]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| val | 0=正常，1=反转 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取编码器极性 | `/einv 1` → `CH1 enc_invert=0` |
| 2 | 设置编码器极性 | `/einv 1 1` → `OK (RAM only)` |

> **用途**：统一「顺时针为正」语义。不同电机个体的编码器方向可能相反，`/inv`（引脚反转）只改变旋转方向，编码器计数方向需 `/einv` 修正（软件取反，即开即用）。

<a id="cmd-posangle"></a>
##### **/posangle** - 位置环转一圈脉冲数

**`/posangle <ch> [val]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| val | 位置环输出轴转一圈脉冲数（物理线数，固件自动 ×4）；0=使用编码器 CPR | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取当前值 | `/posangle 1` → `CH1 pos_angle_cpr=0` |
| 2 | 设置 | `/posangle 1 600` → `OK (RAM only)` |

> **输出轴语义（v2.x）**：`pos_angle_cpr` = 输出轴（减速后）一圈物理线数，减速比 g = `pos_angle_cpr / encoder_cpr`
> （与底盘 wheel_gear 同语义）。速度环恒以输入轴（电机轴）RPM 闭环，PID 增益与减速比无关；
> `pos_angle_cpr` 仅用于图表显示轴换算（输出轴 = 输入轴 ÷ g）与输出轴 RPM 显示（`/check` ROT 行）。
> v2.x 起命令单位概念已删除，命令/目标恒为输入轴口径。减速电机必须正确配置 `encoder_cpr` 与
> `pos_angle_cpr` 两个值，否则输出轴速度/角度换算失真。

#### 编码器/滤波指令（2 条）

<a id="cmd-enczero"></a>
##### **/enczero** - 编码器清零

**`/enczero <ch>`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |

清零指定通道编码器累计计数（同时重置 PID 和位置环状态）。返回 `OK`。

<a id="cmd-filter"></a>
##### **/filter** - 速度滤波器

**`/filter <ch> [type] [window]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| type | 0=无，1=滑动平均，2=一阶低通，3=中值滤波 | 可省略（不传=读取） |
| window | 滑动平均/中值 1~32，一阶低通 1~99 | 可省略（不传=保持当前窗口） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 0 | 显示全部 4 通道 | `/filter` → 四通道配置 |
| 1 | 读取指定通道 | `/filter 1` → `CH1 filter=无 window=1` |
| 2 | 设滤波器类型 | `/filter 1 1` → `OK CH1 filter=滑动平均 window=1 (RAM only)` |
| 3 | 设类型 + 窗口 | `/filter 1 1 8` → 滑动平均窗口 8 |

#### USART2 通讯指令（3 条）

<a id="cmd-uart2"></a>
##### **/uart2** - USART2 配置

**`/uart2 [baud] [inv] [mode]`**

三个参数独立控制，互不覆盖。

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| baud | 波特率 1200~4000000 | 可省略（不传=保持当前值） |
| inv | 极性：0=正常，1=反相（默认反相，SBUS 标准） | 可省略（不传=保持当前值） |
| mode | `uart`=二进制协议解析（默认），`sbus`=SBUS 帧解析，`elrs`=ELRS/CRSF 帧解析（自动置波特率 420000） | 可省略（不传=保持当前值） |

> **注意：** 极性取决于硬件接线方式。信号+GND 双线接入时，GND 接高电平则极性反向，需设 `inv=1`。与是否 SBUS 模式无关。

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 0 | 读取全部 | `/uart2` → `Baud=100000 Inv=1(反相) Mode=SBUS` |
| 1 | 设波特率 | `/uart2 115200` |
| 2 | + 极性 | `/uart2 100000 1` |
| 3 | + 模式 | `/uart2 100000 1 sbus` |

<a id="cmd-priority"></a>
##### **/priority** - 控制优先级

**`/priority [0|1]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| 0 | USART2 优先（默认，遥控器主控） | 可省略（不传=读取） |
| 1 | USB 优先（上位机主控） | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 0 | 读取当前优先级 | `/priority` → `USART2` |
| 1 | 设置优先级 | `/priority 1` → `OK (RAM only)` |

<a id="cmd-timeout"></a>
##### **/timeout** - 指令超时保护

**`/timeout [ms]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| ms | 无控制指令超时时间；超时后所有电机输出归零；0=关闭保护。同时作为优先级心跳窗口（最小 100ms）：优先端口失联超过该时间后非优先端口可接管，0 时按 100ms 计 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 0 | 读取当前超时 | `/timeout` → `500 ms` |
| 1 | 设置超时 | `/timeout 1000` → `OK (RAM only)` |

#### 遥控参数指令（6 条，SBUS / ELRS 共用）

<a id="cmd-smap"></a>
##### **/smap** - 遥控通道映射

**`/smap <ch> [sbus_ch]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| sbus_ch | SBUS 通道号 1~16 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取通道映射 | `/smap 1` → `CH1 SBUS_CH=1` |
| 2 | 设置映射 | `/smap 1 3` → 电机 A 绑定遥控通道 3 |

<a id="cmd-rmap"></a>
##### **/rmap** - 映射模式

**`/rmap <ch> [0|1]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| val | 0=中心零点（双向，默认），1=min零点（单向） | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取映射模式 | `/rmap 1` → `CH1 mapmode=0 (中心零点(双向))` |
| 2 | 设置映射模式 | `/rmap 1 1` → `OK (RAM only)` |

> **两种模式：** 中心零点以通道中心为 0 点，输出 `[-1,+1]` 双向行程；min零点以下边界为 0 点，只输出 `[0,+1]` 正向行程，配合方向映射（`/dmap`）实现正反转。

<a id="cmd-dmap"></a>
##### **/dmap** - 方向映射

**`/dmap <ch> [enable] [dir_ch]`**

把一个开关通道映射为电机的正/反转选择（只改遥控信号方向，不改物理引脚方向，物理方向由 `/inv` 控制）。

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| enable | 0=不启用（透传），1=启用 | 可省略（不传=读取；只给 enable 时不修改通道） |
| dir_ch | 方向开关通道号 1~16 | 可省略（不传=保持当前通道） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取方向映射 | `/dmap 1` → `CH1 dirmap=OFF dir_ch=ch1` |
| 2 | 只设使能 | `/dmap 1 1` → `OK (RAM only)` |
| 3 | 设使能 + 通道 | `/dmap 1 1 6` → 电机 A 启用方向映射，绑定通道 6 |

> **开关语义：** 方向通道在中心（±16 容差）→ 关闭电机（输出 0）；上边界 → 正转；下边界 → 反转。两档开关 = 恒正/恒反，三档开关 = 正转/停止/反转。
> **底盘模式：** 按运动向量轴解释——ch1/2/3 分别对应 vx/vy/ω（ch4 预留无效），方向通道在中心 → 该轴关闭。

<a id="cmd-dinv"></a>
##### **/dinv** - 遥控默认反转

**`/dinv <ch> [0|1]`**（固件 ≥ v1.2.12 / 上位机 ≥ v2.28）

通道级**静态**方向反转，配置后上电即生效，不依赖遥控器开关（适配不同遥控器的摇杆方向约定）。与方向映射（`/dmap`）**乘法叠加**：`电机速度 = 底层计算速度 × 默认反转(±1) × 方向开关(±1/0)`。

- 电机模式：ch 1~4 = 电机 A~D；底盘模式：ch 1~3 = vx/vy/ω（ch4 保留无效，与 `/dmap` 一致）
- 只映射遥控信号方向，不改 `/inv` 物理引脚方向；上位机 0x31/0x30 直控不受影响

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 通道选择 1-4（含义随电机/底盘模式） | 必填 |
| val | 0=正常，1=默认反转 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取默认反转 | `/dinv 1` → `CH1 dinv=OFF` |
| 2 | 设置默认反转 | `/dinv 3 1` → `OK (RAM only)` |

| 默认反转 | 方向开关 | 实际输出方向 |
|:---:|:---:|:---:|
| 0 | 未启用 | 正向（透传，默认） |
| 0 | 上 / 下 | 正转 / 反转 |
| 1 | 未启用 | 反转 |
| 1 | 上 / 下 | 反转 / 正转（叠加） |
| × | 中心 | 关闭（输出 0，方向映射原语义） |

<a id="cmd-sbusparam"></a>
##### **/sbusparam** - SBUS 行程映射

**`/sbusparam <ch> [val]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| \<ch> | 电机通道选择 1-4 | 必填 |
| val | 摇杆满偏对应目标值，含义取决于控制模式：速度=最大 RPM，位置=最大角度（0.1° 单位），开环=不生效 | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 1 | 读取当前值 | `/sbusparam 1` → `CH1 sbusparam=10000 [当前模式=速度, 此值=最大RPM]` |
| 2 | 设置 | `/sbusparam 1 5000` → `OK (RAM only) [当前模式=速度, 此值=最大RPM]` |

<a id="cmd-sbusrange"></a>
##### **/sbusrange** - 通道值范围上下界（SBUS / ELRS 共用）

**`/sbusrange [min] [max]`**

| 参数符号 | 参数意义 | 可否省略 |
|:--:|------|:--:|
| min | 通道值下边界 (100~2047) | 可省略（不传=读取） |
| max | 通道值上边界 (min<max≤2047) | 可省略（不传=读取） |

| 参数数量 | 行为 | 示例 |
|:--:|------|------|
| 0 | 读取当前范围 | `/sbusrange` → `范围: 192~1792 (中心=992)` |
| 2 | 设置范围 | `/sbusrange 172 1811` → `OK (RAM only)`（只给一个参数返回用法错误） |

#### 底盘指令（1 条）

<a id="cmd-chassis"></a>
##### **/chassis** - 底盘构型与参数

**`/chassis [sub [args...]]`**（无参=查询全部；带子命令=设置；RAM only，需 `/save` 持久化；底盘功能说明见 §4.8）

| 子命令 | 参数 | 说明 |
|--------|------|------|
| `type` | 0~6 | 构型：0=禁用（单电机模式）1=差速2WD 2=差速4WD四驱 3=麦轮O 4=全向3WD 5=全向4WD-X 6=全向4WD-十字 |
| `wheel` | 1~65535 | 轮径（mm） |
| `geo` | `<0-2> <v>` | 几何（mm / 0.1°）：差速=轮距/轴距，麦轮=轴距/轮距，全向=半径/基准角 θ0 |
| `max` | `<0-2> <v>` | 限幅（vx/vy: mm/s，ω: 0.001 rad/s）——遥控满偏/指令限幅/斜坡上限一份值三用 |
| `accel` | `<0-2> <v>` | 加速度（vx/vy: mm/s²，ω: 0.001 rad/s²，分轴独立） |
| `stall` | 0~60000 | 失速判定时长 ms（0=禁用失速协调） |
| `stop` | — | 整车停止（斜坡归零） |

查询输出示例：

```
> /chassis
TYPE=差速2WD WHEEL=65mm
GEO=200/150/0 MAX=1500/0/3.000
ACC=1500/0/3.000 STALL=500ms
GEAR: A=30.0 B=30.0 C=30.0 D=1.0   <有编码器但 pos_angle_cpr=0 的轮 1 个: g=1 未计减速比, 用 /posangle 配置>
MODE: A=速度 B=速度 C=开环(k_ch未标定?) D=开环(k_ch未标定?)
```

- **GEAR 行：** 每轮推导减速比 g = 输出轴一圈线数 ÷ 编码器线数。g=1（未配置）时实车速度会慢减速比倍，配置方法：`/posangle <ch> <线数×减速比>`（如 11 线编码器 + 30:1 减速箱 → `/posangle 1 330`）。
- **MODE 行：** 每轮实际执行档位（开环/速度，位置档按速度执行并标注）；开环轮 k_ch（`/sbusparam` 值 ×0.01）仍为出厂默认 1000 时带 `(k_ch未标定?)` 标注——非零目标会饱和到满 PWM，请先标定或改速度闭环。
- **`type` 切换会复位 PID 状态**（清积分），但**不改写各通道控制模式**。

#### 错误处理

- 无效命令返回 `Unknown command '/xxx'`
- 通道号超出 1~4 返回 `ERR: ch 1-4`
- 模式参数错误返回 `ERR: open/speed/pos`

### 5.3 二进制帧格式（主固件）

用于 ROS / 上位机程序交互。所有多字节字段为**小端序**（LE）。通过 USART6（USB）或 USART2（UART 模式）传输。

```
┌──────┬──────┬──────┬───────────┬──────┐
│ SYNC │ CMD  │ LEN  │  DATA     │ CRC8 │
│ 0xAA │ 1B   │ 1B   │ 0~248 B   │ 1B   │
└──────┴──────┴──────┴───────────┴──────┘
```

- **SYNC:** 固定 `0xAA`
- **CMD:** 命令字
- **LEN:** DATA 段字节数（最大 248 = config_t 大小）
- **CRC8:** 多项式 `0x07`，计算范围 CMD + LEN + DATA

### 5.4 ACK 帧格式

```
成功: 0xAA + CMD + 0x01 + 0x00 + CRC8
失败: 0xAA + CMD + 0x01 + 0xFF + CRC8
受保护: 0xAA + CMD + 0x01 + 0x02 + CRC8
```

| err | 含义 |
|:---:|------|
| `0x00` | 成功 |
| `0xFF` | 失败（LEN 不匹配、字段越界、EEPROM 写入失败等） |
| `0x02` | 受保护字段拒绝（仅 WRITE_FIELD：目标偏移落在受保护头部区域 offset < 11） |

### 5.5 二进制命令一览

| CMD | 名称 | DATA (发送) | DATA (应答) | 说明 |
|:---:|------|------------|------------|------|
| `0x01` | PING | 无 | ACK | 连通性测试 |
| `0x10` | READ_PARAM | 无 | config_t (248B) | 读取全部配置 |
| `0x11` | WRITE_PARAM | config_t (248B) | ACK | 写入全部配置（仅 RAM，受保护字段自动还原） |
| `0x12` | WRITE_FIELD | `[field_id:2B LE][value:nB]` | ACK | 按偏移量写入单个字段（offset < 11 拒绝，返回 0x02，见 §5.7） |
| `0x20` | SAVE_EEPROM | 无 | ACK | RAM 配置写入 EEPROM（约 190ms） |
| `0x21` | LOAD_EEPROM | 无 | ACK | 从 EEPROM 加载到 RAM |
| `0x22` | FACTORY_RESET | 无 | ACK | 恢复出厂默认（RAM+EEPROM） |
| `0x30` | MOTOR_RAW | `[ch:1B][dir:1B][pwm:2B LE]` | 无 | 单通道 PWM 直驱（旁路 PID；底盘模式下会被逆解输出覆盖） |
| `0x31` | MOTOR_CTRL | `[m1~m4:4×4B LE]` | 无 | 核心控制帧（int32_t）：单电机模式=四通道目标；底盘模式=整车向量（见下方说明） |
| `0x32` | MOTOR_JOG | `[ch:1B][rpm:2B LE 有符号]` | 无 | 单轮点动（仅底盘模式生效）：覆盖该物理通道轮目标低速转动，用于验向；rpm=0 清除；400ms 无刷新自动解除 |
| `0x40` | SUBSCRIBE | `[interval_ms:2B LE]` | ACK | 开启状态周期上报（最低 20ms） |
| `0x41` | UNSUBSCRIBE | 无 | ACK | 关闭状态上报 |
| `0x43` | DEBUG_SBUS | `[enable:1B]` | ACK | SBUS 16 通道实时上报开关 |
| `0x44` | DEBUG_SPEED | `[enable:1B]` | ACK | 速度原始值上报开关（STATUS_REPORT 帧扩展） |
| `0x52` | ENTER_BL | 无 | ACK | 软复位进入 Bootloader（上位机一键下载用，见 §7） |
| `0x53` | REBOOT | 无 | ACK | 系统重启（ACK 后延迟 100ms） |
| `0xF0` | STATUS_REPORT | 无（MCU 主动推送） | 见 §5.6 | 周期状态上报（需先 SUBSCRIBE） |
| `0xF1` | DETECT_REPORT | 无（MCU 主动推送） | `[proto:1B][inv:1B][baud:4B LE]` | 协议检测结果上报 |
| `0xF2` | SBUS_DATA | 无（MCU 主动推送） | `[ch0~15:16×2B LE]` | SBUS 16 通道原始值（需 DEBUG_SBUS 开启） |

**MOTOR_CTRL（0x31）说明：** `m1~m4` 为 4 个 int32（小端序），含义随模式：
- **单电机模式**（chassis_type=0）：四通道各自的目标值，含义取决于各通道控制模式（开环=PWM ±1000，速度=RPM，位置=0.1°）；上位机控制面板以 30/50/100ms 可调间隔连续发送该帧。
- **底盘模式**（chassis_type≠0）：p1=vx（mm/s）、p2=vy（mm/s）、p3=ω（0.001 rad/s）、p4 保留（发 0）——整车速度向量，经限幅/斜坡/逆解分配到四轮（见 §4.8）。

### 5.6 STATUS_REPORT 帧格式（0xF0）

需先通过 `SUBSCRIBE`（0x40）开启，以设定间隔周期推送。

**常规模式**（DEBUG_SPEED=0，56B payload）：
```
[enc1~4:4×4B LE] [tgt1~4:4×float LE] [rpm1~4:4×4B LE] [sbus_frame_cnt:4B LE] [sbus_ok_cnt:4B LE]
```

- `enc`: int32_t，编码器累计脉冲数
- `tgt`: float，当前目标值（PWM/RPM/位置）
- `rpm`: int32_t，滤波后实时转速（**恒为输入轴/电机轴 RPM**；输出轴口径 = 输入轴 ÷ 减速比 g，见 §5.7 `pos_angle_cpr`）
- `sbus_frame_cnt`: SBUS 帧组装计数
- `sbus_ok_cnt`: SBUS 帧校验通过计数

**扩展模式**（DEBUG_SPEED=1，72B payload）：
```
[enc:16B] [tgt:16B] [rpm:16B] [rpm_raw:16B] [sbus_frame_cnt:4B] [sbus_ok_cnt:4B]
```

- `rpm_raw`: int32_t[4]，滤波前原始 RPM

### 5.7 配置数据结构（config_t，248B，v2.x）

offset 0~10 为受保护区域（魔数 `0x4D445200`、硬件版本 A/B/C、固件协议版本 D、Bug 修复 E、固件适配硬件主版本、CRC8），共 11 字节，不可通过 WRITE_FIELD 写入（目标偏移 < 11 返回 0x02）。可写字段偏移参考（速度环恒输入轴闭环；底盘字段为 v2.0 新增；offset 247 为 v1.2.12 新增的遥控默认反转）：

| 字段 | offset | 大小 | 说明 |
|------|:--:|:--:|------|
| baud_rate | 11 | 4B | USART2 波特率 (u32) |
| cmd_timeout_ms | 15 | 2B | 超时保护 (u16) |
| comm_flags | 17 | 1B | bit0-3=protocol(1=SBUS,2=UART,3=ELRS), bit4=sbus_inv, bit5=ctrl_priority |
| control_mode | 18 | 1B | 每电机2bit: 0=开环, 1=速度, 2=位置 |
| motor_invert | 19 | 1B | 每电机2bit: bit0=引脚反转(0正常/1反转), bit1=编码器极性(0正常/1反转) |
| encoder_cpr[4] | 20 | 8B | 编码器线数 (u16×4, 电机轴一圈) |
| speed_period_ms[4] | 28 | 4B | 速度环周期 (u8×4, 1~255ms, v2.x 压缩) |
| speed_pid_type | 32 | 2B | 每电机4bit: 0=位置式, 1=增量式 |
| speed_olim[4] | 34 | 8B | 速度环输出限幅 (u16×4, PWM 0~1000) |
| speed_ctrl_params[4] | 42 | 64B | 速度环 Kp/Ki/Kd/Ilim (f32×16) |
| pos_period_ms[4] | 106 | 4B | 位置环周期 (u8×4, 1~255ms, v2.x 压缩) |
| pos_pid_type | 110 | 2B | 每电机4bit: 0=位置式, 1=增量式 |
| pos_ctrl_params[4] | 112 | 64B | 位置环 Kp/Ki/Kd/Ilim (f32×16) |
| pos_olim[4] | 176 | 16B | 位置环输出限幅 (f32×4, RPM, 输入轴口径) |
| pos_angle_cpr[4] | 192 | 8B | 位置环转一圈脉冲数 (u16×4, 输出轴一圈线数, 0=用 encoder_cpr) |
| speed_filter_type | 200 | 2B | 每电机4bit: 0=无,1=滑动平均,2=低通,3=中值 |
| speed_filter_window[4] | 202 | 4B | 滤波窗口 (u8×4) |
| sbus_channel_pack | 206 | 2B | 每电机4bit: 0~15=CH1~16（必绑，无未绑定） |
| rc_dir_ch | 208 | 2B | 方向映射通道（每电机4bit: 0~15=CH1~16） |
| rc_map_mode | 210 | 1B | bit0-3=映射模式(0=中心零点,1=min零点), bit4-7=方向映射使能(每电机1bit) |
| sbus_param[4] | 211 | 8B | 遥控行程 (u16×4): 开环=最大PWM, 速度=最大RPM(输入轴口径), 位置=最大角度(0.1°); 底盘开环=每电机速度→PWM标定系数(×0.01) |
| sbus_range_min | 219 | 2B | 通道值下边界 (u16) |
| sbus_range_max | 221 | 2B | 通道值上边界 (u16) |
| chassis_type | 223 | 1B | 底盘构型 (0=禁用, 1~6 见 §4.8) |
| wheel_diameter | 224 | 2B | 轮径 (mm) |
| chassis_geo[3] | 226 | 6B | 底盘几何 (mm/0.1°) |
| chassis_max[3] | 232 | 6B | 底盘限幅 (vx/vy mm/s, ω 0.001rad/s) |
| chassis_accel[3] | 238 | 6B | 底盘加速度 (vx/vy mm/s², ω 0.001rad/s²) |
| stall_time_ms | 244 | 2B | 失速判定时长 (ms, 0=禁用) |
| motor_map | 246 | 1B | 电机映射 (每电机2bit: 0=A 1=B 2=C 3=D, 模型轮位→物理接线口, 默认 0xE4=ABCD 顺序) |
| rc_dir_default | 247 | 1B | 遥控默认反转 (bit0-3=通道1~4, 1=反转; 电机模式=电机A~D, 底盘=vx/vy/ω; bit4-7=保留) |

> **布局说明：** sizeof 248B，EEPROM 256B 余 8B。早期 247B 配置（无 rc_dir_default）升级固件后自动迁移：CRC 二次校验通过则保留全部用户参数、新字节置 0，升级不掉配置；连旧固件（不支持 248B）的上位机会在参数长度处提示不匹配，属预期。
> **Bootloader 兼容注记：** Bootloader 的硬件版本写入功能仍按旧 12B 头部布局写（offset 11 写 0xFF 作 CRC 失效标记），会覆盖 baud_rate 低字节；CRC 随之失效 → 固件加载时检测到配置无效自动恢复出厂默认，重新配置即可，无其他影响。
> **建议：** 上位机程序优先使用 READ_PARAM → WRITE_PARAM 全量读写，避免依赖 WRITE_FIELD 硬编码偏移（每次协议版本升级布局可能变化）。

### 5.8 版本体系

| 版本 | 格式 | 说明 |
|------|------|------|
| 硬件 | vA.B.C | A=MCU 引脚方案，B=硬件次版本，C=变体号（Bootloader 管理） |
| 固件 | vA.D.E | A=HW_MAJOR，D=协议版本(SW_MAJOR)，E=Bug 修复(SW_PATCH)；build 号不对外显示 |
| 上位机 | vD.F | D=协议版本（须与固件 D 一致），F=Bug 修复 |

**匹配规则：**
- 固件 D ≠ 上位机 D → ❌ 协议不兼容
- 固件 A ≠ 上位机 A → ❌ 硬件不匹配
- 其余情况 → ✅ 兼容

**固件文件格式：** `.bin` 文件末尾附加 16 字节 trailer（刷写时只写入 .bin 本体）：
```
[magic:4B] [hwA:1B] [swD:1B] [swE:1B] [build:1B] [reserved:1B] [fw_size:3B] [crc32:4B]
```
- magic = `0x4D523031`（大端读法 "MR01"）
- crc32 = 固件本体（不含 trailer）的 CRC32，上位机据此做完整性校验

---

## 6. 快速上手

从接线、上电到用上位机 / 遥控器控制电机，按本节顺序操作即可快速跑通；调好参数后可脱机运行（见 §6.5）。

### 6.1 开始前准备

**物料清单：**

| 物料 | 说明 |
|------|------|
| 驱动板 | Motor Driver Controller ×1 |
| 电机 + 编码器 | 最多 4 路，插入 A/B/C/D 连接器（线序见 §3.2） |
| 控制源 | SBUS 接收机（需外部供电）或外部 MCU（UART TX，需共地） |
| 电源 | DC 5.5V ~ 15V（推荐 7.4V / 11.1V / 12V），DC-005 2.1mm **内正外负** |
| USB 线 | Type-C 数据线（CH340 虚拟串口，仅通信、不参与供电） |
| 电脑 | Chrome / Edge 浏览器，打开上位机页面 |

**接线步骤：**

1. 电机 + 编码器插入 A/B/C/D 连接器
2. 控制源插入 RC 接口（无极性，正反均可；SBUS 接收机需额外供电）
3. 电源插入 DC 连接座，上电前需**检查电压与极性**！
4. 需要上位机调试时，USB Type-C 连接电脑

主板接线示意图如下

<div align="center"><img src="image/主板接线图.png" width=500></div>

### 6.2 上电前检查

**接线检查清单：**

| 检查项 | 要求 |
|--------|------|
| 电源极性 | DC-005 **内正外负**；板上无反接保护，接反会损坏电路 |
| 电源电压 | 5.5V ~ 12V |
| 电机线序 | Pin1/Pin6 接反 → 转向相反（交换接线或用 `/inv` 补偿） |
| 编码器线序 | Pin3(ENC_A)/Pin4(ENC_B) 接反 → 方向识别错误（可用 `/einv` 补偿） |
| 控制信号 | RC 已插好；SBUS 接收机已接外部供电 |
| SW1 开关 | 先保持 OFF，插好电源后再拨 ON |

**上电自检：** SW1 拨 ON，观察 LED：

| LED 现象 | 状态 | 后续操作 |
|----------|------|----------|
| 三快闪 → 常亮 | ✅ 正常 | 进入下一步 |
| 慢闪 (1Hz) | ⚠️ 正常，无控制信号 | 接控制源或上位机 |
| 急促快闪 | ❌ 故障报警 | EEPROM 读取失败 / 硬件故障，断电检查接线 |

**LED 状态完整说明：**

| 状态 | 模式 | 时序 | 含义 |
|------|------|------|------|
| 上电初始化 | 三快闪 → 常亮 | 100ms ×3 | EEPROM 加载成功 |
| 无信号等待 | 慢闪 | 500ms ON/OFF | 等待控制信号 |
| SBUS 工作中 | 长亮短灭 | 800ms/200ms | SBUS 正常接收 |
| ELRS 工作中 | 三闪停顿 | 200ms×3/500ms | ELRS/CRSF 正常接收 |
| UART 工作中 | 双闪停顿 | 150ms×2/500ms | 串口指令模式 |
| 识别中 | 交替快闪 | 50ms | FUN 键扫描中 |
| 识别成功 SBUS | 三短一长 | 100ms×3 → 常亮（2s 后切运行态） | 已锁定 SBUS |
| 识别成功 ELRS | 四短一长 | 100ms×4 → 常亮（2s 后切运行态） | 已锁定 ELRS |
| 识别成功 UART | 两短一长 | 100ms×2 → 常亮（2s 后切运行态） | 已锁定 UART |
| 识别失败 | 急闪×5/停顿1s | 100ms×5 | 回退到存储配置 |
| 故障 | 急促快闪 | 100ms | EEPROM 读取失败、硬件故障 |

### 6.3 上位机使用

**连接步骤：**

1. USB Type-C 连接电脑，设备管理器确认 CH340 端口号
2. 使用浏览器打开上位机页面
3. 点击「🔌 未连接」→ 选择 CH340 串口（波特率固定 2000000）
4. 连接成功后点击「📥 同步参数」读取设备配置

> 配图：上位机截图

<div align="center"><img src="image/上位机截图.png" width=600></div>

**界面说明：**

**左面板 — 参数设置区：**
- 版本信息 + 端口连接（悬停 ? 查看版本规则）
- 同步参数 / 校验参数（绿框=已同步设备，红框=已修改未发送）
- 系统设置（命令超时、控制优先级）、USART2 设置（协议/极性/波特率）
- 电机配置（编码器 PPR、方向反转、PWM 输出限幅）
- 速度滤波器设置、电机控制模式
- 底盘设置（v2.0）：构型预设、轮径/几何/限幅/加速度/失速参数、每轮减速比显示（未配置红色提示）
- 速度环 / 位置环 PID 参数（四通道独立）
- RC 遥控：通道映射、映射模式、方向映射、默认反转、行程映射、通道值范围
- 底部：发送至下位机 / 保存到 EEPROM / 固件更新

**悬浮入口：** 控制面板（四通道推杆 / 底盘向量控制）、PID 自整定（🧪，见下）、电机映射示意图（点轮即验向：长按轮子低速点动，配合转向/编码器极性修正）。

**右面板：** 四通道 RPM 实时曲线（输入/输出轴口径可切换）、时间轴滚动窗口、串口终端（HEX/文本、净屏过滤、指令补全）。

**操作流程：**

1. 连接串口
2. 「📥 同步参数」→ 从设备读取配置
3. 修改参数（改过的变红色）
4. 「📤 发送至下位机」→ 立即生效（仅 RAM）
5. 确认无误「💾 保存到 EEPROM」→ 持久化（断电不丢失，脱机运行见 §6.5）

> **小技巧：** 「💾 保存到 EEPROM」按钮双击可切换「实时更新模式」（参数修改即时同步）。
> **自整定：** 上位机提供 PID 自动整定面板（🧪），可对速度环 PID 与滤波器做自动整定，需先配置正确的编码器 PPR。

### 6.4 遥控器使用（SBUS / ELRS）

**SBUS 接收机：**

1. SBUS 接收机信号线插入 RC（任意方向），接收机外部供电（RC 不对外供电）
2. 长按 FUN 键 1 秒触发协议识别（LED 交替快闪）
3. LED 三短一长 → 识别成功，进入 SBUS 模式
4. 默认通道映射：CH1~CH4 → 电机 A~D
5. 可用 `/smap`、`/rmap`、`/dmap`、`/sbusparam`、`/sbusrange` 调整映射、模式与行程

**ELRS 接收机：**

1. ELRS 接收机 CRSF pad 输出接入 RC（任意方向），接收机外部供电；默认直通不反相
2. 长按 FUN 键 1 秒触发协议识别（LED 交替快闪）
3. LED 四短一长 → 识别成功，进入 ELRS/CRSF 模式（或手动 `/uart2 420000 0 elrs`）
4. 通道映射/行程映射/通道值范围与 SBUS 完全共用（见下）

- **通道映射（`/smap`）**：同一遥控通道可映射到多个电机（联动）；每电机必须绑定通道，不用该路就物理不接电机
- **映射模式（`/rmap`，v1.0）**：中心零点（双向，默认）/ min零点（单向），单向模式配合方向映射实现正反转
- **方向映射（`/dmap`，v1.0）**：绑定开关通道控制正反转（只改遥控信号方向）：方向通道在中心→电机停；上边界→正转；下边界→反转
- **行程映射（`/sbusparam`）**：摇杆满偏对应的目标值，含义取决于控制模式：
  - 开环：最大 PWM（默认 1000）
  - 速度闭环：最大 RPM（默认 10000）
  - 位置闭环：最大角度（0.1° 单位，3600 = ±360.0°）
- **通道值范围（`/sbusrange`）**：固件只存上下界 min/max（默认标准 192~1792）；上位机下拉提供预设（标准/扩展/CRSF·ELRS）并支持自定义输入

### 6.5 脱机使用

**概念：** 全部参数（控制模式、PID、编码器、滤波、SBUS 映射等）保存在板上 EEPROM。用上位机调好参数并保存后，设备可完全脱离电脑独立运行——每次上电开机自动从 EEPROM 读取配置，并按该配置运行。

**操作流程：**

1. 上位机连接并「📥 同步参数」
2. 调整各项参数（控制模式 / PID / 编码器 PPR / 滤波器 / SBUS 映射等）
3. 「📤 发送至下位机」试运行，观察右侧 RPM 曲线确认效果
4. 确认无误后「💾 保存到 EEPROM」（提示成功后断电不丢失）
5. 断开 USB、关闭电源
6. 重新上电：LED 三快闪 → 常亮，表示 EEPROM 配置加载成功，设备按已保存配置自动运行

**典型场景：** 接上 SBUS 接收机（外部供电）与电源，不连接电脑，直接用遥控器控制电机。

**注意事项：**

- 仅「📤 发送至下位机」的修改只在 RAM 中生效，**断电即丢失**；必须「💾 保存到 EEPROM」才会持久化
- 控制优先级（`/priority`）保持 0（USART2 优先，遥控器主控）即可脱机遥控；1 = USB 优先（上位机主控）
- 协议识别：上电使用 EEPROM 中保存的协议配置（SBUS/UART）；更换控制源后需长按 FUN 键重新识别（识别失败会回退到存储配置）
- 命令超时保护（`/timeout`）：脱机运行时超过设定时间未收到控制指令，电机输出自动归零（0 = 关闭保护）

### 6.6 二次开发例程

配套例程随手册发布于 `examples/` 目录（Python 需 `pyserial`，运行前 `pip install pyserial`）：

| 文件 | 演示内容 | 对应章节 | 所需硬件 |
|------|---------|---------|---------|
| `mdc.py` | 协议最小库：CRC8、帧构建/解析、文本指令收发、遥测解析 | §5 | —（被其他例程 import） |
| `01_ping_version.py` | 连接、PING、读版本 | §5.4 / §5.5 | 驱动板 + USB |
| `02_params.py` | 读全量配置、WRITE_FIELD 改单字段、保存/加载 | §5.5 / §5.7 | 驱动板 + USB |
| `03_motor_control.py` | 单电机开环/速度/位置三模式控制 | §4.1 | 驱动板 + 电机 + USB |
| `04_chassis_control.py` | 底盘配置 + 整车 (vx, vy, ω) 向量控制 | §4.8 / §5.5 | 装成底盘的驱动板 |
| `05_telemetry.py` | 订阅并解析实时遥测（56B/72B 两种帧） | §5.6 | 驱动板 + 电机 + USB |
| `mdc_mcu_example.ino` | 外部 MCU（Arduino 框架）经 RC 口控制：文本指令 + 二进制帧两种写法 | §3.3 / §5 | 外部 MCU ↔ RC 线 |
| `README.md` | 接线图、运行方法、安全提示 | §6 | — |

**片段预览 —— 底盘整车控制（Python，完整版见 `04_chassis_control.py`）：**

```python
from mdc import Mdc            # pip install pyserial
import time

mdc = Mdc("COM5")              # USB 虚拟串口，固定 2000000-8N1
# 底盘配置（RAM 生效，确认后 /save 持久化；轮径/减速比务必先配好）
mdc.cmd("/chassis type 1")     # 1=差速2WD（3=麦轮O 4=全向3WD 5=X 6=十字）
mdc.cmd("/chassis wheel 65")   # 轮径 65mm
mdc.cmd("/chassis max 0 300")  # vx 限幅 300mm/s（0/1/2 = vx/vy/ω）
mdc.cmd("/save")

# 整车向量：vx=+200mm/s 直行 2 秒（底盘模式下 0x31 = vx/vy/ω，见 §5.5）
mdc.motor_ctrl(200, 0, 0, 0)   # 库内自动组帧：AA 31 10 + 4×int32 小端 + CRC8
time.sleep(2)
mdc.motor_ctrl(0, 0, 0, 0)     # 停
```

> **安全提示：** 首次联调请将底盘架空（轮子离地）；确认 `/timeout` 超时保护已开启；先小限幅后大限幅。

---

## 7. 固件更新（OTA）

固件通过 Bootloader 经 USB 串口在线刷写，无需额外编程器。

### 7.1 进入 Bootloader（一键下载 / 手动兜底）

BL v1.0+ 支持三条自动进 BL 路径：

| 路径 | 触发方式 | 适用场景 |
|------|---------|---------|
| ① RTC magic | 上位机点「开始下载」→ 发 `CMD_ENTER_BL` (0xAA/0x52) → APP 写 RTC 后备寄存器 magic 并软复位 | 设备运行中（一键下载主路径） |
| ② 物理按键 | 按住 FUN 键 → 上电（或按 RST 复位）→ 松开 FUN | 手动兜底 |
| ③ CDC 握手 | 上位机在设备上电后 200ms 内发握手帧 `[0xA5,0x55,0]` | 设备未上电 / 冷启动 |

进入成功后设备输出 `BL:BOOT` 文本标记，上位机自动识别并切换至 Bootloader 模式（连接按钮显示「✅ Bootloader 模式」）。

### 7.2 刷写流程（一键下载）

1. 打开上位机「🔄 固件更新」
2. 拖入固件 `.bin` 文件（或点击选择）
3. 点「⚡ 开始下载」— 上位机**自动进入 Bootloader**，无需手动按键
4. 上位机自动执行：**ERASE（擦除）→ WRITE（写入）→ VERIFY（CRC32 校验）→ REBOOT（重启）**
5. 完成后设备自动重启进入新固件

### 7.3 Bootloader 协议（BL 帧）

**指令头 = 0xA5，无 CRC**。

```
BL 帧: [SYNC=0xA5] [CMD] [LEN] [data...]   (无 CRC)
```

| CMD | 值 | 说明 |
|-----|-----|------|
| CMD_READ | 0x10 | 读 EEPROM 版本（应答 6B：`[0xA5][0x10][0x03][hwA][hwB][hwC]`） |
| CMD_ERASE | 0x50 | 擦除 App 区 |
| CMD_WRITE | 0x51 | 写 Flash（`[addr:4B LE][data...]`，每块 128B，起始地址 0x08004000） |
| CMD_VERIFY | 0x52 | 校验（`[size:3B LE][crc32:4B LE]`） |
| CMD_REBOOT | 0x53 | 重启进 App |
| CMD_BL_REQ | 0x55 | 上位机握手帧 `[0xA5,0x55,0]` — 设备上电后 200ms 窗口内收到即停留 BL |
| ACK | 0x5A | 应答帧头 |

**ACK 帧格式**（无 CRC）：`[0xA5][0x5A][0x01][err]`，`err=0x00` 为成功，`err=0xFF` 为失败（NAK）。

**典型超时参数：** 擦除 20s、单块写入 500ms、校验 5s。

---

## 8. 故障排查

| 现象 | 可能原因 | 排查步骤 |
|------|----------|----------|
| LED 不亮 | 无供电 | 检查电源、SW1 开关 |
| 上电 LED 急促快闪 | 硬件故障 | 断电，检查电机线短路、编码器线序、测试EEPROM是否正常 |
| 电机不转 | 无控制信号 | 接上位机 `/check` 确认，或换 SBUS 测试 |
| 电机抖动 | PID 参数不当 | 减小 Kp，增大 Kd，或切回开环测试硬件 |
| 编码器读数为 0 | 编码器供电线序错误 | 检查电机接口线序是否与控制板一致，特别是3V3和GND |
| 编码器读数跳跃 | 信号干扰、编码器缺陷 | 降低编码器线长，使用速度滤波器 |
| 电机转向反了 | 接线反了 | 交换 Pin1/Pin6 或用 `/inv <ch> 1` |
| 上位机连不上 | COM 口错误 | 设备管理器确认 CH340 端口号；波特率必须为 2000000 |
| SBUS 遥控无反应 | 未识别协议 | 长按 FUN 键重新识别 |
| 保存后断电丢失 | EEPROM 写入失败 | 再执行一次 `/save`，确认提示 "OK" |
| 上位机提示版本不兼容 | 固件/上位机版本不匹配 | 查看 §5.8 匹配规则，升级固件或上位机 |

**电气安全注意事项：**
- 请勿过压、反接：接错就BOOM了
- 电压不超过 15V，电机电流不超过 TB6612 额定（单路持续 1.2A / 峰值 3.2A）

---

## 9. 附录

### A. 指令速查表

| 分类 | 指令 | 主要参数 |
|------|------|------|
| 系统 | `/help` `/version` `/status` `/check` `/detect` `/save` `/load` `/reset` | — |
| 控制器 | `/speedctrl` | `<ch> [kp ki kd [ilim [olim [period [ptype]]]]]` |
| 控制器 | `/posctrl` | `<ch> [kp ki kd [olim [ilim [period [ptype]]]]]` |
| 控制器 | `/mode` | `<ch> [open\|speed\|pos]` |
| 控制器 | `/cpr` | `<ch> [val]` |
| 控制器 | `/inv` | `<ch> [0\|1]` |
| 控制器 | `/einv` | `<ch> [0\|1]` |
| 控制器 | `/posangle` | `<ch> [val]` |
| 编码器 | `/enczero` | `<ch>` |
| 滤波 | `/filter` | `<ch> [type] [w]` |
| UART2 | `/uart2` | `[baud] [inv] [mode]` |
| UART2 | `/priority` | `[0\|1]` |
| UART2 | `/timeout` | `[ms]` |
| 遥控 | `/smap` | `<ch> [sbus_ch]` |
| 遥控 | `/rmap` | `<ch> [0\|1]` |
| 遥控 | `/dmap` | `<ch> [enable] [dir_ch]` |
| 遥控 | `/dinv` | `<ch> [0\|1]` |
| 遥控 | `/sbusparam` | `<ch> [val]` |
| 遥控 | `/sbusrange` | `[min] [max]` |
| 底盘 | `/chassis` | `[type \| wheel \| geo \| max \| accel \| stall \| stop]`（详见 §5.2） |

### B. 术语表

| 术语 | 含义 |
|------|------|
| CPR | Cycles Per Revolution，编码器每转物理刻度数 |
| PPR | Pulse Per Revolution，同 CPR |
| SBUS | Futaba S-BUS，串行总线协议，100kbps 8E2 反相逻辑 |
| PID | 比例-积分-微分控制器 |
| 位置式 PID | 输出为绝对控制量 |
| 增量式 PID | 输出为增量变化量 |
| OTA | Over-The-Air，本手册指经串口在线刷写固件 |
| BL | Bootloader |
| config_t | 248 字节配置结构体（EEPROM 持久化，v2.x） |
| 底盘构型 | 整车轮布局模型（差速/麦轮/全向轮等，chassis_type 0~6，见 §4.8） |
| 减速比 g | 输出轴一圈线数 ÷ 编码器线数（`/posangle` 配置；未配置 =1，实车速度慢 g 倍） |
| 失速协调 | 底盘按最慢轮能力比缩放整车目标的保护机制（§4.8） |

### C. 参考资料

- TB6612 数据手册
- STM32F401 数据手册
- Futaba S-BUS Technical Specification

### D. 手册修订记录

| 版本 | 时间 | 修订内容 |
|------|------|---------|
| v1.0 | 2026-08-04 | 初版：产品概述、技术规格与电气属性（含极限参数）、硬件接口、固件功能、通信协议、使用指南、OTA、故障排查、附录 |
| V2.0 | 2026-08-12 | 适配 ELRS 遥控协议（CRSF 帧格式）：支持 SBUS、CRSF 双遥控协议控制；`/uart2` 新增 `elrs` 模式；config_t 重构（250B→253B，`protocol` 字段新增 3=ELRS）；`/sbusrange` 由三档预设改为上下界 min/max 存储；LED 状态与识别指示同步扩展（ELRS 工作中/识别成功 ELRS） |
| V2.1 | 2026-08-12 | 遥控通道扩展：config_t 全面位宽打包 253B→231B（comm_flags 位域、control_mode/motor_invert 每电机2bit、pid_type/filter_type 每电机4bit、遥控通道 nibble 打包）；新增映射模式（中心零点/min零点，`/rmap`）与方向映射（`/dmap`，中心停/上正/下反，只改信号方向）；开环行程可配（sbus_param=最大PWM）；新增 `/rmap` `/dmap` 指令；上位机 RC 区块新增映射模式/方向映射配置；BL 头部由 12B 变 11B（offset 11 交还固件，BL 写 0xFF 靠 CRC 重置兜底） |
| V2.x | 2026-08-30 | 速度环输出轴跟踪 + EEPROM 压缩：speed/pos_period_ms 由 uint16 压缩为 uint8（省 8B）、新增 `speed_fb_unit`（每电机 1bit，0=电机轴RPM 1=输出轴RPM），config_t 255B→248B 余 8B；速度环反馈单位可选输出轴 RPM（`输出轴RPM = 电机轴RPM × encoder_cpr / pos_angle_cpr`，复用 pos_angle_cpr 推导减速比，与底盘 wheel_gear 同语义；底盘模式强制电机轴）；新增 `/speedunit` 指令；`/posangle` 明确为输出轴一圈线数语义；上位机新增速度环追踪对象配置入口（协议 v2） |
| V2.2 | 2026-08-31 | **删除速度跟踪输出轴（命令单位）**：移除 `speed_fb_unit` 字段与 `/speedunit` 指令，config_t 248B→247B 余 9B；速度环恒以输入轴(电机轴)RPM 闭环（与 v1.2.2 语义一致），命令/目标恒为输入轴口径，PID 增益与减速比无关；`pos_angle_cpr` 仅用于图表显示轴换算与 `/check` ROT 显示（输出轴 = 输入轴 ÷ g）；上位机移除「速度命令单位」配置与自整定输出轴通道处理，图表显示轴切换保留（手动）；协议版本 D 保持 2（配套固件 v1.2.3 / 上位机 v2.13） |
| V2.3 | 2026-08-31 | **修复底盘模式遥控方向映射无效**：`/dmap` 方向映射按运动向量轴生效（vx/vy/ω，方向通道中心→该轴关闭），与单电机路径同一判区语义（固件 v1.2.4）；协议/配置布局不变（D=2 不变，上位机无需升级） |
| V2.4 | 2026-08-31 | **修复麦轮/全向轮底盘转速超快**：运动学逆解单位换算错误——麦轮/全向轮公式输出轮角速度 rad/s 被当作线速度 m/s 换算电机轴 RPM，超速 2/轮径(米) 倍（60mm 轮≈33 倍），拉杆微动即撞 12000 RPM 上限；改为直接传轮线性速度（半径抵消），差速不受影响（固件 v1.2.5）；协议/配置布局不变 |
| V2.5 | 2026-08-31 | **减速比可视化（防底盘速度偏慢）**：减速比 g=pos_angle_cpr/encoder_cpr 仅在转一圈脉冲数>0 时生效（默认 0 → g=1，电机轴转速=轮速，实车速度慢减速比倍）；`/chassis` 查询新增 GEAR 行（每轮 g + 未配置提示，固件 v1.2.6）；上位机 v2.16 底盘区块新增只读减速比显示（红色「未配×1」+ 配置引导）；排查底盘速度异常需同时核对轮径（wheel_diameter 误配同样会拖慢/加快车速） |
| V2.6 | 2026-09-09 | **修复底盘预设≠0 强制切速度闭环**：旧版固件底盘执行层只按编码器有无分发（有编码器的轮恒为速度闭环、忽略控制模式），且 `/chassis type` 与上位机选预设都强制把有编码器的轮切到速度闭环——开环在底盘模式不可用、切回单电机后模式残留。固件 v1.2.10 起底盘每轮执行按 control_mode 分发（开环=开环 PWM / 速度闭环=速度环，位置档按速度；无编码器仍强制开环），`/chassis type` 不再改写控制模式，`/chassis` 查询新增 MODE 行（每轮实际执行档位 + k_ch 未标定标注）；上位机 v2.24 起选预设不再改写控制模式，开环+k_ch 未标定弹一次性提醒。协议 D=2 不变；⚠ 旧配置「有编码器+开环模式+依赖底盘隐式速度环」升级后按开环执行，k_ch（/sbusparam，默认 10000）未标定时会满 PWM，升级后先查 `/chassis` MODE 行 |
| v1.0（对外发布） | 2026-08-15 | **首次对外发布**：内部 V2.x 内容对外统一为 v1.0 版本体系（固件 v1.0.x / 上位机 v1.0.x / 手册 v1.0，硬件 v1.1.x），协议版本 D=1；本手册作为首个对外版本发布 |
| v2.0（对外发布） | 2026-09-11 | **v2 线首个对外发布**：内部迭代至固件 v1.2.13 / 上位机 v2.38 后重置对外版本号——固件 v1.2.0（A=1, D=2, E=0）/ 上位机 v2.0（D=2, F=0）/ 手册 v2.0，协议版本 D=2；底盘功能线（差速/麦轮/全向轮运动学、底盘预设、底盘执行按 control_mode 分发）随本版对外发布 |
| V2.7 | 2026-09-11 | **手册与源码对齐维护 + 配套例程**：文本指令集补全至 27 条（新增 `/rmap` `/dmap` `/dinv` `/chassis` 完整条目与速查表）；新增 §4.8 底盘功能章节（构型 0~6、参数语义、执行档位与减速比、三种控制途径）；协议勘误——ACK 受保护错误码 0x02、WRITE_FIELD 保护边界 offset<11、命令表补 0x32 MOTOR_JOG / 0x52 ENTER_BL、MOTOR_CTRL 补底盘向量语义；config_t 更新为 248B 实际布局（rc_dir_default@247，247B 旧配置自动迁移）；§6.3 上位机界面补底盘设置/自整定/映射示意图；新增 §6.6 二次开发例程 + `examples/` 目录（Python×6 + Arduino×1 + README）。协议版本 D=2 不变，手册对外版本号不变 |
| V2.8 | 2026-09-11 | **尺寸口径勘误**：§5.5 命令表 READ_PARAM/WRITE_PARAM 的 config_t 尺寸 253B→248B、§5.3 帧最大 LEN 250→248（源码校验 `len > sizeof(config_t)`，v1.2.0 实际 248B，与 §5.7 一致）。纯文档勘误，协议版本 D=2 不变 |
