# -*- coding: utf-8 -*-
"""
mdc_lite.py — 极简调用库（只管调用 / send-only，Python 参考实现）
=================================================================

**核心思想：上位机调参、下位机执行。** 你只需要告诉下位机"发什么控制量"，
本模块把"要发送的字节"打包好交给你，由你自行 `ser.write()` 发送。
...
**各函数返回 `bytes`（完整二进制帧，含 SYNC+CRC8），直接 ser.write 即可。**
"""
...
def subscribe(interval_ms: int) -> bytes:
