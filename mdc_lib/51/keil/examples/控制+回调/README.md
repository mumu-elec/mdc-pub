...
> **目录：** `mdc_lib/51/keil/examples/控制+回调/`
> **定位：** 演示「发控制帧 + 收速度回调」：用 `md_lite_ctrl` 发送 `0x31 MOTOR_CTRL`，用 `md_lite_ctrl` 的接收器流式解析下位机主动推送的 `0xF0 STATUS_REPORT`，把四通道 `rpm[4]` 回调给用户。
> **用的库：** [`mdc_lib`](../../../../51/keil/mdc_lib.h)（底层，已内置）+ [`mdc_lite`](../../../../51/keil/mdc_lite.h) + [`mdc_lite_ctrl`](../../../../51/keil/mdc_lite_ctrl.h)（send-only + 回调接收，内含 mdc_lite）。
...
`mdc_lite_ctrl` 发送侧 = `mdc_lite`（= `mdc_lib` 的薄封装）；接收侧自带流式 0xF0 解析（不依赖 `mdc_lib`）。字节布局、CRC8、帧格式与 mdc_lib 完全一致。若只需发送，引 `mdc_lite` 即可；要回读转速，引 `mdc_lite_ctrl`（同时获得发送 + 回调）。
