...
> **目录：** `mdc_lib/51/keil/examples/极简控制/`
> **定位：** 只演示「发送控制帧」：用 `mdc_lite` 打包 `0x31 MOTOR_CTRL` 并逐字节发出。**不解析任何回包**——若需回读 0xF0 转速，用 [`../控制+回调/`](../控制+回调/)（`mdc_lite_ctrl`）。
> **用的库：** [`mdc_lib`](../../../../51/keil/mdc_lib.h)（作底层，已内置）+ [`mdc_lite`](../../../../51/keil/mdc_lite.h)（极简 send-only）。
...
`mdc_lite` 是**独立实现**（自带 CRC8/组帧，不依赖 `mdc_lib`）：`md_lite_ctrl` 为独立实现（自带 CRC8/组帧）、`md_lite_subscribe` == `md_bin_subscribe`、`md_lite_unsubscribe` == `md_bin_unsubscribe`。字节布局、CRC8（0x07, 初值 0）、帧格式与 mdc_lib 完全一致，只是把关注范围收窄到「发送控制」这 4 条命令，不含文本指令 / config_t / SBUS。
