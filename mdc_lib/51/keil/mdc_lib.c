 ...
 *
 * Protocol basis: 协议规范.md (layout v2.x, config_t = 248B)
...
#if MD_ENABLE_CONFIG
/* 248B scratch buffer for the config pack path. On Keil C51 SMALL model the
                            ...
                            uint16_t value_len, uint8_t* out, uint16_t cap)
{
    ...
    if (value_len > (uint16_t)(MD_MAX_DATA - 2))   /* 2B field_id + value */
        return 0;
    if (field_id < 11)                             /* protected region (offset<11) not writable (FW replies ACK 0x02) */
        return 0;
    if (out == 0)
        return 0;
...
uint16_t md_bin_motor_jog(uint8_t ch, int16_t rpm, uint8_t* out, uint16_t cap)
{
    ...
    out[2] = 3;
    out[3] = ch;
    out[4] = (uint8_t)((uint16_t)rpm & 0xFF);          /* int16 LE, signed */
...
int md_parse_config(const uint8_t* raw, uint16_t len, md_config_t* out)
{
    ...
    /* chassis (new in v2.x, offset 223~247) */
