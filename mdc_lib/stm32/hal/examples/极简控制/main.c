/*
 * ============================================================================
 *  极简控制（send-only）—— STM32 HAL 最小示例
 * ============================================================================
 *  演示用 mdc_lite（极简调用库）打包并发送控制帧：
 *    0x31 MOTOR_CTRL / 0x40 SUBSCRIBE / 0x41 UNSUBSCRIBE。
 *  只发送，不解析任何回包（若需回读转速，请用 ../控制+回调/）。
 *
 *  本例只写最小 main.c，不 include 任何 HAL 头文件 —— 串口收发在真实工程里
 *  把 user_send() 的实现替换为你工程的实际发送即可（如 HAL_UART_Transmit）。
 *
 *  库文件（来自 mdc_lib/stm32/hal/）：mdc_lib.h/.c、mdc_lite.h/.c
...
/* 串口发送由你实现。真实工程替换为：
 *   HAL_UART_Transmit(&huart2, buf, n, 100);   （huart2 为你的 CubeMX 句柄） */
...
int main(void)
{
    ...
    for (;;) {
        ...
        /* 真实工程：HAL_Delay(50);  或用定时器 50ms 周期 */
    }
