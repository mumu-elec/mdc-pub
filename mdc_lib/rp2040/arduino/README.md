...
- **纯 C++ 打包/解析库**：不 include `Arduino.h` / pico 头文件，与框架无关（本机 g++ `-std=c++17 -Wall -Wextra` 可直接编译校验）。
...
- **不碰串口**：串口收发由用户实现 —— 拿库返回的字节 `Serial1.write(buf, n)` 发送；收到的字节喂给 `md_parser_feed`。
...
```cpp
/* MotorDriver_RP2040.ino —— 串口收发由用户实现，库只负责打包/解析 */
#include "mdc_lib.h"
...
3. 发送：`uint16_t n = md_bin_xxx(..., buf, sizeof(buf)); Serial1.write(buf, n);`
...
| 收不到数据 | arduino-pico 的 Serial1 引脚随核心版本变化，用 `setTX`/`setRX` 显式指定 |
| 文本指令无响应 | 确认带 `\n`（库已自动加） |
