# mdc_lite — RP2040（Arduino，arduino-pico）极简调用库接入文档

> 平台：Raspberry Pi Pico 等 RP2040 板（arduino-pico core）｜ 文件：`mdc_lite.h` + `mdc_lite.cpp`（send-only）、`mdc_lite_ctrl.h` + `mdc_lite_ctrl.cpp`（调用+回调）
...
- **纯 C++**：不 include 任何 Arduino/pico 头文件，本机 g++ `-std=c++17 -Wall -Wextra` 可直接编译校验。

> 复用约定：本目录 `mdc_lite.h`/`mdc_lite.cpp`/`mdc_lite_ctrl.h`/`mdc_lite_ctrl.cpp` 与 esp32 / avr / esp8266 **内容完全一致**，仅本 README 的接线/串口示例不同。
...
## 三、串口接入示例（RP2040 → Serial1）

驱动器 USART2（RC 口）需**先**通过任意终端配置为 UART 模式并共地：`/uart2 115200 0 uart`。

```cpp
/* MotorDriver_RP2040_lite.ino —— 串口收发由用户实现，库只负责打包/回调 */
...
void setup() {
    ...
    /* 接驱动器 USART2。arduino-pico 的 Serial1 默认引脚因核心版本而异
     * （多数版本默认 TX=GPIO4 / RX=GPIO5），可用 setTX/setRX 显式指定 */
    Serial1.setTX(4);
    Serial1.setRX(5);
...
3. 发送：`uint16_t n = md_lite_xxx(..., buf, sizeof(buf)); Serial1.write(buf, n);`
