# mdc_lite — ESP8266（Arduino）极简调用库接入文档

> 平台：ESP8266（Arduino core）｜ 文件：`mdc_lite.h` + `mdc_lite.cpp`（send-only）、`mdc_lite_ctrl.h` + `mdc_lite_ctrl.cpp`（调用+回调）
...
- **纯 C++**：不 include 任何 ESP8266 头文件，本机 g++ `-std=c++17 -Wall -Wextra` 可直接编译校验。

> 复用约定：本目录 `mdc_lite.h`/`mdc_lite.cpp`/`mdc_lite_ctrl.h`/`mdc_lite_ctrl.cpp` 与 esp32 / rp2040 / avr **内容完全一致**，仅本 README 的接线/串口示例不同。
...
## 三、串口接入示例（ESP8266 → Serial）

驱动器 USART2（RC 口）需**先**通过任意终端配置为 UART 模式并共地：`/uart2 115200 0 uart`。

```cpp
/* MotorDriver_ESP8266_lite.ino —— 串口收发由用户实现，库只负责打包/回调 */
...
> 上电瞬间 GPIO1/3 会输出 boot 日志（文本），回调接收器会将其当作噪声丢弃，不影响后续二进制帧解析。
...
3. 发送：`uint16_t n = md_lite_xxx(..., buf, sizeof(buf)); Serial.write(buf, n);`
