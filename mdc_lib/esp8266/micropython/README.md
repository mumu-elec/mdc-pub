# mdc_lib — MicroPython 实现（ESP8266）
...
| UART1 | 仅 TX=GPIO2，**无 RX** | 只能发送（适合纯下发控制/配置，无法接收上报） |

因此本 README 的主示例用 **UART0 + 释放 REPL** 实现双向通信；附录给出 UART1 纯发送用法与软串口（bit-bang）说明。
...
控制板 USART2（RC 口）需先配置为 UART 模式（通过 USB 口发一次）：
```
/uart2 115200 0 uart
```
之后 RC 口即可收发文本指令与二进制帧，波特率与本示例的 115200 保持一致。

> 说明：控制板 **USB 口固定 2000000-8N1**（CH340N 虚拟串口），波特率不可改；RC 口波特率由 `/uart2` 决定。ESP8266 波特率建议 ≤ 115200（软件栈较慢，高波特率易丢字节）。

## 三、串口接入示例（UART0，TX=GPIO1 / RX=GPIO3，释放 REPL）
...
# ① 释放 REPL 对 UART0 的占用（把 REPL 重定向回 USB 串口；此后不能再从 UART0 用 REPL）
os.dupterm(None, 1)

# ② 初始化 UART（用户实现收发；库本身不碰任何硬件）
uart = UART(0, baudrate=115200, timeout=50)
...
uart1.write(mdc_lib.md_bin_motor_ctrl(100, 0, 0, 0))      # 0x31 控制帧
uart1.write(mdc_lib.md_text_mode(1, "speed"))             # 文本指令
# 注意：UART1 没有 RX，无法接收 STATUS_REPORT / ACK，适合单向控制
```

### 附录 B：软串口（bit-bang）说明

若必须保留 UART0 给 REPL 又要双向通信，可自选两个 GPIO 用软件实现 UART（如 bit-bang 或社区 SoftUART 库）。
库侧用法完全不变——软串口收到的字节同样喂给 `MDParser`，发送用 `uart.write(mdc_lib.md_bin_xxx(...))`。
软串口波特率建议 ≤ 57600，且注意关闭中断/定时器冲突以保证时序。
...
| UART0 被 REPL 占用 | 先 `import os; os.dupterm(None, 1)` 释放；或改用 UART1（仅发送）/ 软串口 |
| 发控制帧电机不动 | 检查 `/priority`：RC 口默认优先（/priority 0）；USB 上位机实时控制需先 `/priority 1`；检查 `/timeout` 未超时归零 |
| 控制帧时灵时不灵 | 协议识别（`/detect` 或 FUN 键）期间控制帧被拒绝，等待识别完成；ESP8266 建议把波特率降到 115200 以下 |
| 文本指令无响应 | 确认带 `\n`（本库 `md_text_*` 返回均含 `\n`）；部分终端需 `\r\n` |
