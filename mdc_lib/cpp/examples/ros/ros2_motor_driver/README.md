...
| `/get_config` | `ros2_motor_driver/srv/GetConfig` | 服务：空请求 → 响应 `uint8[248] config`（config_t 全量 248B，小端） |
| `/set_config` | `ros2_motor_driver/srv/SetConfig` | 服务：请求 `uint8[248] config` → 响应 `bool success`（仅 RAM，受保护字段自动还原） |
...
**mdc_lib.hpp 已随本包内置（`include/mdc_lib.hpp`），开箱即用**：`CMakeLists.txt` 的 include 路径已指向本包 `include/` 目录，无需任何拷贝或额外配置；如需更新库版本，用 `../../../../cpp/mdc_lib.hpp` 覆盖本包 `include/mdc_lib.hpp` 即可。
