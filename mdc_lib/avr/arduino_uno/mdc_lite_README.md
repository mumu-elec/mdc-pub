# mdc_lite — Arduino UNO（ATmega328P）极简调用库接入文档

> 平台：Arduino UNO（ATmega328P，16MHz）｜ 文件：`mdc_lite.h` + `mdc_lite.cpp`（send-only）、`mdc_lite_ctrl.h` + `mdc_lite_ctrl.cpp`（调用+回调）
...
- **纯 C++**：不 include 任何 Arduino 头文件，本机 g++ `-std=c++17 -Wall -Wextra` 可直接编译校验。

> 复用约定：本目录 `mdc_lite.h`/`mdc_lite.cpp`/`mdc_lite_ctrl.h`/`mdc_lite_ctrl.cpp` 与 esp32 / rp2040 / esp8266 **内容完全一致**，仅本 README 的接线/串口示例不同。
...
void on_speed(const int32_t rpm[4]) {
    ...
    Serial.print(rpm[1]); Serial.print(','); Serial.print(rpm[2]); Serial.print(',');
    Serial.println(rpm[3]);
}
```

## 三、串口接入示例（UNO → SoftwareSerial）

驱动器 USART2（RC 口）需**先**通过任意终端配置为 UART 模式并共地：`/uart2 115200 0 uart`。

```cpp
/* MotorDriver_UNO_lite.ino —— 串口收发由用户实现，库只负责打包/回调 */
#include "mdc_lite_ctrl.h"          /* 同时获得 send-only + 回调 */
#include <SoftwareSerial.h>

static md_lite_ctrl_t g_ctrl;       /* 极简控制器（内含流式解析器，全局/static 避免占栈） */

/* SoftwareSerial(rxPin, txPin)：UNO RX=10 ← 控制板 RC TX，TX=11 → 控制板 RC RX */
SoftwareSerial rcSerial(10, 11);
...
3. 发送：`uint16_t n = md_lite_xxx(..., buf, sizeof(buf)); rcSerial.write(buf, n);`
...
## 六、内存说明（UNO 仅 2KB SRAM）

- `md_lite_ctrl_t` 内含 mdc_lib 流式解析器（`MD_PARSER_BUF` 默认 256B），请声明为**全局/static**，避免占栈。
- 仅收发 0x31 / 0x40 / 0x41 / 0xF0 的极简场景，256B 缓冲绰绰有余（最大收发帧 20B）。若想裁剪 `MD_PARSER_BUF`，必须通过编译选项 `-DMD_PARSER_BUF=xxx` **同时作用于 mdc_lib.cpp / mdc_lite_ctrl.cpp**，否则编译单元间结构体布局不一致会导致解析器越界——不建议初学者修改。
- SoftwareSerial 在 115200 下偶发乱码：可把 `rcSerial.begin(115200)` 与 `/uart2` 降到 **9600**，或改用硬件 UART（D0/D1）。
