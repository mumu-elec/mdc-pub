...
1. 把 `mdc_lib.py` 复制到 ESP32 文件系统（Thonny / ampy / WebREPL 均可）。
2. **接线**（ESP32 UART2，3.3V 逻辑电平，与控制板**共地**）：
...
> 说明：控制板 **USB 口固定 2000000-8N1**（CH340N 虚拟串口），波特率不可改；RC 口波特率由 `/uart2` 决定。本示例走 RC 口（USART2），波特率 115200。

## 三、串口接入示例（UART2，TX=GPIO17 / RX=GPIO16）
