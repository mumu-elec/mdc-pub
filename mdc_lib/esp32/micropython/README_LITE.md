...
1. 把 `mdc_lite.py`、`mdc_lite_ctrl.py`（按需）复制到 ESP32 文件系统（Thonny / ampy / WebREPL 均可），与你的 `main.py` **同目录**。`mdc_lite` 是独立实现，无需 `mdc_lib.py`；仅当需要完整能力（文本指令 / config 读写等）时才一并复制 `mdc_lib.py`。
2. **接线**（见 §4.1），与控制板**共地**。
