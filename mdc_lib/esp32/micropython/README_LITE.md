# mdc_lite — 极简调用库（MicroPython 实现：ESP32）

> 在完整版 `mdc_lib` 之上的一层**极简封装**，只服务于一个核心场景：
> **上位机调参、下位机执行** —— 你只需告诉下位机要发什么控制量，并从下位机拿回实时转速。
> 除此之外的文本指令、config_t 全字段读写、SBUS/检测/波特率识别等**一律不管**。
> **零依赖**：复用本目录 `mdc_lib.py`（`md_bin_motor_ctrl` / `md_bin_subscribe` / `md_bin_unsubscribe` / `MDParser` / `md_parse_status`），本身不 import `machine` / `struct` / `math`，串口收发由你实现。
> 规范依据：[`../../LITE.md`](../../LITE.md)；底层协议：[`../../../例程/common/协议规范.md`](../../../例程/common/协议规范.md)。
...
| `mdc_lib.py` | 完整版底层库（API.md），mdc_lite 依赖它，无需改动 |
...
## 四、串口接入示例（UART2，TX=GPIO17 / RX=GPIO16）

### 4.1 接线（ESP32 UART2，3.3V 逻辑电平，与控制板**共地**）

| ESP32 | 控制板 |
|-------|--------|
| GPIO17（UART2 TX） | RC 口信号（USART2 RX） |
| GPIO16（UART2 RX） | RC 口信号（USART2 TX） |
| GND | GND（必须共地） |

> 控制板 **USB 口固定 2000000-8N1**（CH340N），波特率不可改；**RC 口（USART2）** 波特率由 `/uart2` 决定，本示例走 RC 口、波特率 115200。
...
1. 把 `mdc_lib.py`、`mdc_lite.py`、`mdc_lite_ctrl.py`（按需）复制到 ESP32 文件系统（Thonny / ampy / WebREPL 均可），与你的 `main.py` **同目录**。
2. **接线**（见 §4.1），与控制板**共地**。
...
- mdc_lite **不重复实现协议**：只复用本目录 `mdc_lib.py` 的打包/解析原语。
- 字节布局（LE）、CRC8（0x07，初值 0）、帧格式 `[AA][CMD][LEN][DATA][CRC8]` 与 `mdc_lib` **完全一致**；`mdc_lite.ctrl(...)` 等价于 `mdc_lib.md_bin_motor_ctrl(...)`。
- 一个工程只需发送 → 引 `mdc_lite`；需回读转速 → 引 `mdc_lite_ctrl`（同时获得发送与回调）。二者底层都复用同一份 `mdc_lib`。
