...
```cpp
/* MotorDriver_ESP32.ino —— 串口收发由用户实现，库只负责打包/解析 */
#include "mdc_lib.h"
...
void loop() {
    ...
    while (Serial2.available() > 0) {
        ...
        if (md_parser_feed(&g_parser, b, &cmd, &payload, &plen) == 1) {
            ...
            } else if (cmd == 0xF1) {                     /* DETECT_REPORT */
                md_detect_t dt;
                if (md_parse_detect(payload, plen, &dt) == 1) {
                    Serial.printf("proto=%d baud=%lu\r\n",
                                  dt.proto, (unsigned long)dt.baud);
                }
            }
...
3. 发送：`uint16_t n = md_bin_xxx(..., buf, sizeof(buf)); Serial2.write(buf, n);`
