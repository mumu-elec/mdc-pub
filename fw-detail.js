/* ============================================================
 * fw-detail.js — 固件发布详情页共享渲染器
 * 数据源: 站点根 releases.json (单一事实来源, 见 SPEC.md §3/§4)
 * 壳页只需: window.MDC_FW_DETAIL = '<固件版本>'; 然后引入本文件
 * ============================================================ */
(function(){
  'use strict';
  var FW = window.MDC_FW_DETAIL;
  ...
  /* ====== releases.json 基址 = 本脚本所在目录 (壳页位于 fw/ 子目录也能正确定位站点根)
  ...
  fetch(base+'releases.json',{cache:'no-cache'}).then(function(r){
    ...
    var line=null,rel=null;
    (m.lines||[]).forEach(function(l){
      (l.releases||[]).forEach(function(r){if(r.fw===FW){line=l;rel=r;}});
    });
    if(!rel){errBox('未在版本清单中找到固件 v'+FW+' — 请确认 releases.json 已包含该版本的 detail 字段');return;}

    var h=rel.host||{};
    var cur=!!line.current;
    document.title='固件 v'+rel.fw+' — Motor Driver Controller 发布详情';

    var html='<div class="wrap">'+
      '<a class="back" href="'+base+'index.html">← 返回发布页</a>'+
      '<div class="hero">'+
        '<h1>固件 <span class="fwv">v'+esc(rel.fw)+'</span></h1>'+
        '<div class="meta">'+
          ...
          '<span class="chip">发布 '+dateOrDash(rel.date)+'</span>'+
        '</div>'+
      '</div>';

    /* 1. 版本信息 */
    html+='<section><h2>'+ic('file','hi-acc')+'版本信息</h2><div class="kv">'+
      '<span class="k">固件版本</span><span class="v">v'+esc(rel.fw)+'（vA.D.E · A=硬件 D=协议 E=补丁）</span>'+
      '<span class="k">所属线</span><span class="v">'+esc(line.name)+' · 协议 D='+line.protocol+'</span>'+
      '<span class="k">发布日期</span><span class="v">'+dateOrDash(rel.date)+'</span>'+
      '<span class="k">配套上位机</span><span class="v">'+(h.ver?'v'+esc(h.ver):'—')+'</span>'+
      '<span class="k">状态</span><span class="v">'+(cur?'当前维护':'存档 · 不再更新')+'</span>'+
      '</div></section>';

    /* 2. 更新内容 */
    var pts=(rel.highlights&&rel.highlights.length)?rel.highlights:(rel.notes?[rel.notes]:[]);
    html+='<section><h2>'+ic('bolt','hi-acc')+'更新内容</h2>';
    if(pts.length){
      html+='<ul class="plain">';
    ...
    html+='<section><h2>'+ic('download','hi-acc')+'下载</h2>'+
      flist('固件 v'+esc(rel.fw),[
        frow(rel.fw_bin,'固件二进制（上位机刷写用）',rel.date),
        frow(rel.fw_hex,'HEX 格式（烧录器直刷）',rel.date)
      ]);
    if(h.ver){
      html+=flist('配套上位机 v'+esc(h.ver),[
          frow(h.offline,'离线版，无检查更新与固件在线下载功能',rel.date),
          frow(h.online,'联网版，含在线固件库 / 检查更新，断网自动降级',rel.date)
        ])+
        (h.web?'<div class="btns" style="margin-top:10px"><a class="btn sm ghost" href="'+esc(h.web)+'">在线使用</a></div>':'');
    }
    html+='</section>';

    /* 4. 手册与资料 (版本级 resources 优先, 缺省回退线级 — 版本对应规则见主仓 docs/资料发布规范.md) */
