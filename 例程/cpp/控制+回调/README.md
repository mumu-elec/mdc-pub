...
> **分类：** 例程/cpp/控制+回调　|　**库：** `mdc_lite_ctrl`（调用+回调接收，极简调用库）
...
`mdc_lite_ctrl` 复用之 `mdc_lite`（发送侧）与 `mdc_lib`（`mdc::Parser` 流式解析器 + `mdc::parse_status`），**不改变协议字节布局**（LE / CRC8 0x07 初值0 / `[AA][CMD][LEN][DATA][CRC8]`）。它是「send-only」之上多加了「0xF0→速度回调」的接收侧。若一个工程只需发送，可只引 `mdc_lite.hpp`（见「极简控制」）；需回读转速时引 `mdc_lite_ctrl.hpp` 即可同时获得发送与回调。
