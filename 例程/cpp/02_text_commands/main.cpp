// ============================================================================
// 例程 02: text_commands —— 文本指令交互终端
//
// - 交互循环: 从 stdin 读一行 → 发送（自动补 \n）→ 打印回显
...
// 发送一条文本指令并读取、打印回显
static void sendAndEcho(const std::string& raw) {
    std::string cmd = raw;
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (line == "1")        sendAndEcho("/version");
        else if (line == "2")   sendAndEcho("/check");
        else if (line == "3")   sendAndEcho("/status");
        else if (line == "4")   sendAndEcho("/mode 1 speed");
        else if (line == "5")   sendAndEcho("/mode 1 open");
        else if (line == "6")   sendAndEcho("/save");
        else                    sendAndEcho(line);          // 原样当作指令发送
    }
