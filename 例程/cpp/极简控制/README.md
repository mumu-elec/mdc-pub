...
> **分类：** 例程/cpp/极简控制　|　**库：** `mdc_lite`（send-only，极简调用库）
...
1. 拷贝 `mdc_lite.hpp`（连同 `mdc_lib.hpp`）到你的工程（本目录已内置，开箱即用）；
...
`mdc_lite` 是 `mdc_lib`（API.md 完整版）之上的一层**极简封装**，直接复用本平台 `mdc_lib` 的 `mdc::bin_motor_ctrl` / `mdc::bin_subscribe` / `mdc::bin_unsubscribe`，**不改变协议字节布局**（LE / CRC8 0x07 初值0 / `[AA][CMD][LEN][DATA][CRC8]`），也不重复实现协议。本程序只用发送侧；需要回读转速时请引 `mdc_lite_ctrl.hpp`（见「控制+回调」）。
...
- 与 `mdc_lib` 打包结果逐字节一致（库已自测，见 `mdc_lib/cpp/test_mdc_lite.cpp`）
