 ...
 *    recv:  md_lite_ctrl_init / md_lite_ctrl_feed  (reuses md_parser_feed +
 *           md_parse_status from mdc_lib)
 *  This file only implements the UART I/O (user side).
 *
 *  Tuning (must be before the include): MD_ENABLE_CONFIG=0 compiles out the
 *  config full-field functions + 231B xdata. MD_PARSER_BUF is the parser
 *  buffer size (default 256); keep it large enough for the longest frame you
 *  receive (the 72B 0xF0 STATUS_REPORT frame is 76B, so MD_PARSER_BUF >= 76).
 *  IMPORTANT: MD_PARSER_BUF sets the size of md_parser_t, so if you change it
 *  you must set it project-WIDE (Options -> C51 -> Define) - not only here -
 *  so main.c and mdc_lib.c see the same struct layout. This example keeps the
 *  default 256; add MD_ENABLE_CONFIG=0 (+ optionally MD_PARSER_BUF=96) to the
...
/* mdc_lib/mdc_lite state.
 * The md_parser_t buffer (MD_PARSER_BUF bytes) must live in xdata on 8051
 * (internal RAM is only 128B in SMALL model). tx_buf fits a 0x31 frame (20B). */
...
void uart_isr(void) interrupt 4
{
    if (RI) {
        RI = 0;
        md_lite_ctrl_feed(&g_parser, SBUF);
    }
    if (TI) {
        TI = 0;
    }
...
void main(void)
{
    ...
    md_lite_ctrl_init(&g_parser, on_speed);
