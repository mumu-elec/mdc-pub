/*
 * ============================================================================
 *  Motor Driver Controller — Arduino UNO 极简控制例程（mdc_lite send-only）
 * ============================================================================
 *  只用 mdc_lite：只打包"要发送的控制帧"，不解析任何回包。
 *  硬件：Arduino UNO（ATmega328P，16MHz）
 *  连接：SoftwareSerial(10, 11) 接控制板 RC 接口（USART2）
 *        - UNO RX = 10 ← 控制板 RC TX；UNO TX = 11 → 控制板 RC RX；GND 共地
...
void setup()
{
    ...
    Serial.println(F("=== Motor Driver Controller - UNO (mdc_lite 极简控制/send-only) ==="));
