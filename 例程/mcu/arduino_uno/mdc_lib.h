 ...
 *       本机 g++（-std=c++17 -Wall -Wextra）可直接编译校验。
 *       串口收发由用户实现：拿返回的字节自己 SerialX.write(buf, n) 发送；
 *       收到的字节喂给 md_parser_feed()（流式）或 md_parse_frame()（帧级）解析。
 *
 * 四平台（esp32/arduino、rp2040/arduino、avr/arduino_uno、esp8266/arduino）的
 * mdc_lib.h / mdc_lib.cpp 内容完全一致，仅各平台 README 的接线 / 串口示例不同。
 *
 * 字节序：所有多字节字段小端序（LE）；int32/uint16 一律手动移位拼装，无结构体对齐依赖。
 *
 * 用法示例（.ino 中，串口由用户实现）：
 *   #include "mdc_lib.h"
 *   uint8_t buf[64];
 *   uint16_t n = md_bin_motor_ctrl(100, 0, 0, 0, buf, sizeof(buf));
 *   Serial2.write(buf, n);                 // 发送：库返回的字节
 *
 *   // 接收：loop / 中断里逐字节喂解析器
 *   while (Serial2.available()) {
 *       uint8_t b = (uint8_t)Serial2.read();
 *       if (md_parser_feed(&g_parser, b, &cmd, &payload, &plen)) {
 *           // 收到完整且 CRC 通过的一帧（cmd / payload / plen）
 *       }
 *   }
 */
...
/* config 全字段函数开关（小内存平台置 0 可编译掉以省 RAM/代码，
...
typedef struct {
    ...
    float    tgt[4];          /* @16  当前目标值（PWM/RPM/位置） */
...
#if MD_ENABLE_CONFIG
/* config_t 全字段结构（API.md §6.5，偏移见协议规范 §5）。
 * 位域约定：control_mode/motor_invert 每电机 2bit；speed_pid_type/pos_pid_type/
 *           speed_filter_type 每电机 4bit；sbus_channel/rc_dir_ch 存储 1~16
 *           （解析 +1 / 打包 -1）；rc_map_mode 每电机 1bit，rc_dir_en 由同一
 *           字节 bit4-7 拆出。 */
typedef struct {
    /* 通讯 */
    uint32_t baud_rate;          /* @11 USART2 波特率 */
    uint16_t cmd_timeout_ms;     /* @15 超时保护（ms），0=关闭 */
...
/* 流式解析器状态（API.md §3.4）。
 * 缓冲较大（默认 256B），请声明为全局 / static，避免占栈 */
typedef struct {
...
/* 解析 ACK：输入可为 ACK 帧 DATA 段（1 字节 err，此时 cmd 置 0），
 * 也可为完整 ACK 帧（AA CMD 01 err CRC，自动识别并校验 CRC）。
 * 成功返回 1，失败返回 0。err=0x00 成功，任何非 0 视为失败 */
