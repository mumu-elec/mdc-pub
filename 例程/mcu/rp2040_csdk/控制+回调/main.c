/*
 * ============================================================================
 *  控制 + 回调（control + speed callback）—— RP2040 C SDK 最小示例
 * ============================================================================
 ...
 *
 *  本文件不 include 任何 pico-sdk 头文件 —— 串口收发在真实工程里替换 user_send()
 *  与 UART 接收中断（用 hardware/uart.h 读字节后喂 md_lite_ctrl_feed）即可。
 *
 *  库文件（来自 mdc_lib/rp2040/c-sdk/）：mdc_lib.h/.c、mdc_lite.h/.c、mdc_lite_ctrl.h/.c
 *  库：本目录 CMakeLists 已把三者打包为 mdc_lib 静态库。
 *
 ...
 * 真实工程在 UART0 接收中断里 uart_getc 后逐字节喂入。 */
static void user_rx_poll(md_lite_ctrl_t* c)
{
    (void)c;
    /* 真实工程：
     *   while (uart_is_readable(uart0))
     *       md_lite_ctrl_feed(c, uart_getc(uart0));
     */
}
...
int main(void)
{
    ...
    for (;;) {
        ...
        user_rx_poll(&ctrl);       /* 也可在 UART0 接收中断里直接喂 */
