 ...
 *          以便与 51/Keil 版本保持核心一致，同时可被任意 C99/C11 工具链编译。
 */
...
/* ==================== 内部工具：多字节小端手动拼装 / 拆解 ====================
 * 说明：所有移位前先提升到目标无符号宽度，保证 16 位 int 平台（51）上行为一致；
 *       float 位模式通过 memcpy 搬运，避免类型别名与对齐问题。 */
                   ...
                   uint8_t* cmd, const uint8_t** payload, uint16_t* payload_len)
{
    ...
    ln = p->buf[2];
    if (ln > MD_MAX_DATA) {          /* LEN 非法：丢弃该帧起点，重扫 */
        md_parser_drop_first(p);
        return 0;
    }
