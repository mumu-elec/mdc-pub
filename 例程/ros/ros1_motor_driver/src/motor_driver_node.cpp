...
public:
    ...
    MotorDriverNode()
    {
        ...
        if (status_interval_ms_ < 20) {
            ROS_WARN("status_interval_ms 小于设备上报下限 20ms, 已钳位为 20");
...
private:
    ...
    void startup()
    {
        ...
        if (send_priority_cmd_) {
            ...
            if (echoed) {
                ...
                ROS_WARN("2s 内未收到 /priority 1 回显, 请检查设备/接线; 已收到: \"%s\"",
                         echo.c_str());
            }
        } else {
    ...
    void handle_frame(uint8_t cmd, const std::vector<uint8_t>& payload)
    {
        if (cmd == mdc::MD_CMD_STATUS_REPORT) {               // 0xF0: 设备主动推送
            ...
            ROS_DEBUG("忽略主动上报帧 CMD=0x%02X LEN=%zu", cmd, payload.size());
    ...
    void handle_status(const std::vector<uint8_t>& payload)
    {
        ...
        if (!mdc::parse_status(payload, st)) {
            ROS_WARN("STATUS_REPORT 解析失败 (len=%zu, 期望 56/72)", payload.size());
            return;
        }
                        ...
                        ros1_motor_driver::SaveConfig::Response& resp)
    {
        ...
        resp.success = (r == REPLY_ACK_OK);
        ROS_INFO("SAVE_EEPROM(0x20) %s (写入 EEPROM 约 190ms)",
